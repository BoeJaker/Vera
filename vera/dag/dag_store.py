...
class ExecutionMonitor:
    ...
    async def _correct_errors(
        ...
        from Vera.vera.capability_orchestration import _flatten_dag
...
@capability(
    ...
    description="Search capabilities by semantic relevance to a query. "
                "WHEN TO USE: you need a tool for a task but don't know its exact name — 'find tools for sending email', "
                "'what caps handle image generation', 'search for database query tools'. "
                "Input: query (str!), top_k (int default 20), category (str — filter by category). "
                "Output: {results: [{name, description, signature, score, tags}], count} — ranked by relevance.",
)
