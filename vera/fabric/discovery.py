...
# ── Browser-realistic request headers + UA rotation ────────────────────────
# Bot walls (Imperva/Incapsula "Pardon Our Interruption", Cloudflare, etc.)
# fingerprint thin header sets. We send a full, self-consistent modern-Chrome
# header block (incl. client hints + Sec-Fetch-*) and can rotate the UA.
_UA_POOL = [
...
def _browser_headers(custom_ua: str = "", rotate: bool = False,
                     referer: str = "") -> Dict[str, str]:
    """Return a full, self-consistent modern-browser header block. `custom_ua`
    overrides everything; otherwise the default (or a random pool UA if
    `rotate`) is used. Chromium UAs get matching Sec-CH-UA client hints."""
    if custom_ua:
        ua = custom_ua
    elif rotate:
    ...
    # Client hints only make sense for Chromium engines.
    if "Chrome/" in ua and "Firefox/" not in ua:
        ...
        try:
            ...
            pass
        brand = ('"Microsoft Edge";v="%s", "Chromium";v="%s", "Not.A/Brand";v="24"' % (ver, ver)
                 if "Edg/" in ua else
                 '"Google Chrome";v="%s", "Chromium";v="%s", "Not.A/Brand";v="24"' % (ver, ver))
        h["Sec-CH-UA"] = brand
        h["Sec-CH-UA-Mobile"] = "?0"
        h["Sec-CH-UA-Platform"] = ('"Windows"' if "Windows" in ua
                                    else '"macOS"' if "Mac OS" in ua else '"Linux"')
    if referer:
        h["Referer"] = referer
    return h


# Default header block (back-compat name used across the module).
_HTTP_HEADERS = _browser_headers()


# ── Lazy bridges into the sibling fabric modules ────────────────────────────
def _df():
    import Vera.vera.fabric.data_fabric as df
    return df
...
async def extract_entities_for_record(text: str, record_id: str, dataset_id: str,
    ...
    `llm_struct` lets a caller supply an already-computed structure
    ({entities, relations}, e.g. from the unified _llm_page_analyze pass) so we
    reuse it instead of making a second _llm_structure call for the same page.
    """
    ...
    try:
        ...
        if llm_assist:
            try:
                struct = llm_struct if llm_struct is not None else \
                    await _llm_structure(text, content_type)
    ...
    try:
        ...
        conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS fabric_collections (
                ...
                name          TEXT,
                root_url      TEXT,
                dataset       TEXT,          -- materialised <name>.collection.<slug>
                entity_type   TEXT,          -- graph node label for each entry
                field_map     TEXT,          -- JSON: canonical field map / extraction plan
                mode          TEXT,          -- heuristic|template_llm|per_page_llm|auto
                detail_urls   TEXT,          -- JSON list of detail-page URLs
                done_urls     TEXT,          -- JSON list of URLs already reconstructed
                row_count     INTEGER DEFAULT 0,
                status        TEXT,           -- detected|running|done|error
                note          TEXT,
                created_at    TEXT,
                updated_at    TEXT
            );
...
async def _run_crawl_impl(crawl_id: str, ds_id: str, seed_url: str, config: Dict,
    ...
    # Queue entry: (priority, seq, url, depth, dry_streak, parent_url)
    # priority = -score so heapq (min-heap) pops highest-relevance items first.
    # seq is a monotonic tiebreaker so URLs are never compared as strings.
    _pq_seq = 0
    ...
    heapq.heapify(queue)   # safe on empty list and on already-heap lists
    ...
    # Optional GOAL \u2014 what the user wants to BUILD from the crawl. When set it
    ...
    if brief_on:
        try:
            brief = await _gen_brief(topic, goal=goal) or {}
            ...
            await _emit("brief", topic=topic, goal=goal, brief=brief,
                        message=f"brief: {brief.get('type','?')} \u2014 must: "
                                ...
                                + (" | goal: " + brief.get("goal", "") if brief.get("goal") else ""))
        except Exception as e:
    ...
    proc_q: "asyncio.PriorityQueue" = asyncio.PriorityQueue(maxsize=0)  # unbounded — never blocks fetch path
    ...
    async def _proc_worker():
        ...
        while True:
            ...
            try:
                ...
                # When BOTH tagging and LLM entity extraction are wanted, do them
                # in ONE structured LLM call (relevance+tags+entities+relations)
                # instead of two round-trips for the same page. The extracted
                # structure is handed to extract_entities_for_record so it skips
                # its own _llm_structure call.
                _shared_struct = None
                if do_tag and topic and do_ent and llm_ent:
                    try:
                        pa = await _llm_page_analyze(jtext, jtitle, topic, jck)
                        if pa:
                            tags = pa.get("tags") or []
                            if tags or pa.get("relevance") is not None:
                                _patch_record_tags(jds, jrid, tags, pa.get("relevance"))
                            if tags:
                                await _emit("llm_action", action="tagged", url=jurl,
                                            tags=tags[:8],
                                            message="LLM tags: " + ", ".join(tags[:8]))
                            # only reuse as the entity structure if it found any;
                            # otherwise let extract_entities_for_record retry
                            if pa.get("entities"):
                                _shared_struct = {"entities": pa.get("entities") or [],
                                                  "relations": pa.get("relations") or []}
                    except Exception as e:
                        log.debug("bg page-analyze %s: %s", jurl, e)
                elif do_tag and topic:
                    try:
                ...
                if do_ent:
                    try:
                        if llm_ent and _shared_struct is None:
        ...
        async def _refine_brief():
            for _r in range(brief_rounds):
                ...
                try:
                    ...
                    nb = await _gen_brief(topic, samples=samples, prev=brief, goal=goal)
                    if nb:
                        brief.update(nb)
                        ...
                        for t in nb.get("avoid_terms", []):
                            neg_tokens.update(_words(t))
                        config["topic_brief_data"] = brief
                        await _emit("brief_refined", round=_r + 1, goal=goal, brief=brief,
                                    message=f"brief refined (round {_r+1}) \u2014 must: "
                                            + ", ".join(nb.get("must_terms", [])[:6])
                                            + (" | goal: " + nb.get("goal", "") if nb.get("goal") else ""))
                except Exception as e:
                    log.debug("brief refine: %s", e)
        _brief_refiner = asyncio.create_task(_refine_brief())

    # Anti-bot: per-crawl browser headers. `user_agent` overrides the UA;
    # `ua_rotate` assigns a realistic UA PER DOMAIN (not per page) — a real
    # browser keeps one UA per site, so a consistent per-domain identity looks
    # natural while still varying the fingerprint across the many domains a
    # crawl touches. Configurable from the discovery UI.
    _crawl_ua     = str(config.get("user_agent", "") or "").strip()
    _crawl_rotate = bool(config.get("ua_rotate", False))
    _dom_ua: Dict[str, str] = {}   # netloc -> chosen UA (stable per domain)
    def _ua_for(netloc: str) -> str:
        ua = _dom_ua.get(netloc)
        if not ua:
            ua = random.choice(_UA_POOL)
            _dom_ua[netloc] = ua
        return ua
    _crawl_headers = _browser_headers(custom_ua=_crawl_ua, rotate=_crawl_rotate)

    async with httpx.AsyncClient(timeout=20, follow_redirects=True,
                                  headers=_crawl_headers) as client:
        async def _process(page_url, depth, dry, parent_url):
            ...
            try:
                async with (await _host_fetch_slot(urlparse(page_url).netloc)):
                    ...
                    # Per-domain UA (stable within a site) + same-origin Referer so
                    # the request looks like in-site navigation, not a bot.
                    _ph = None
                    if _crawl_rotate and not _crawl_ua:
                        _ph = _browser_headers(custom_ua=_ua_for(urlparse(page_url).netloc),
                                               referer=parent_url or page_url)
                    resp = await client.get(page_url, headers=_ph) if _ph else await client.get(page_url)
                ...
                if _url_ext in _NON_HTML_FILE_EXTS or "application/" in ct:
                    ...
                    if _parent_path and _parent_path != _parsed_pu.path:
                        ...
                        if _pnorm not in visited and depth > 0:
                            visited.add(_pnorm)
                            heapq.heappush(queue, (-0.4, _next_seq(), _parent_url, max(0, depth - 1), False, page_url))
            ...
            queued_norm = {_normalize_url(q[2]) for q in queue}
            added = 0
            _enq_cap = max_links_pp if max_links_pp else 10 ** 9
            # Children of highly-relevant pages get a priority lift so
            # expansion of a good wiki/doc section happens immediately.
            _parent_boost = 0.3 * relevance if relevance > 0.7 else 0.0
            for sc, lu, lhost in scored_links:
                ...
                _prio = -min(1.0, sc + _parent_boost)
                heapq.heappush(queue, (_prio, _next_seq(), lu, cdepth, next_dry, page_url))
                queued_norm.add(lu)
                added += 1
            ...
            if _ghp and _ghp.group(2).lower() not in _GH_RESERVED:
                ...
                for eu in tabs:
                    ...
                    if eun not in visited and eun not in queued_norm:
                        heapq.heappush(queue, (-0.8, _next_seq(), eun, depth + 1, next_dry, page_url))
                        queued_norm.add(eun)
        ...
        while not _stop and (queue or pending) and pages_fetched < max_pages:
            while (queue and len(pending) < max_conc and
                ...
                _priority, _seq, page_url, depth, dry, parent_url = item
            ...
            if _want_synth:
                try:
                    ...
                    if isinstance(r, dict) and r.get("ok"):
                        ...
                        err_msg = (r or {}).get("error", "synthesis returned no result")
                        log.warning("auto-synthesize %s: %s", ds_id, err_msg)
                        await _emit("synthesize_error", dataset_id=ds_id,
                                    message=f"auto-synthesis failed: {err_msg}")
                except Exception as e:
...
async def cap_discover_crawl(
    ...
    config = {
        ...
        "goal": goal.strip(),
        "user_agent": user_agent.strip(), "ua_rotate": ua_rotate,
        "page_text_cap": page_text_cap, "page_link_cap": page_link_cap,
        "max_record_chars": max_record_chars,
    }
    return await _run_crawl(crawl_id, ds_id, url, config, [(-1.0, 0, url, 0, 0, None)], set())
...
async def cap_discover_continue(
    ...
    try:
        ...
        _raw_q  = json.loads(fr.get("queue") or "[]")
        if _raw_q and isinstance(_raw_q[0][0], str):
            # Migrate old (url, depth, dry, parent) 4-tuple format to new 6-tuple.
            # Assign depth-based priority: shallower = higher priority.
            queue = [(-max(0.0, 1.0 - 0.2 * x[1]), i, x[0], x[1], x[2],
                      x[3] if len(x) > 3 else None)
                     for i, x in enumerate(_raw_q)]
        else:
            queue = [tuple(x) for x in _raw_q]
        # heapq.heapify is called in _run_crawl_impl on entry
    except Exception as e:
...
async def cap_discover_from_dataset(
    ...
    queue = [(-1.0, i, u, 1, 0, None) for i, u in enumerate(frontier_urls)]
    config = {
        "max_pages": max_pages, "max_depth": max_depth, "same_domain": same_domain,
...
@capability(
    "fabric.surfaces.enumerate",
    http_method="POST", http_path="/fabric/surfaces/enumerate",
    http_tags=["fabric", "discover"], memory="off",
    description="Enumerate a discovered API surface (an OpenAPI/Swagger spec or a "
                "REST resource-index) into an api_endpoints sub-table — capturing "
                "the WHOLE API rather than the single endpoint that was detected. "
                "Optionally also promote it into a recurring source (connector). "
                "Input: surface_id (str!), promote (bool=False — also create a "
                "connector), auto_pull (bool=False — pull the connector immediately). "
                "Output: {ok, endpoints, sub_dataset?, parent_dataset, kind, "
                "source_id?, dataset_id?}.",
)
async def cap_surfaces_enumerate(
    surface_id: str = "", promote: bool = False, auto_pull: bool = False,
    trace_id=None,
) -> Dict:
    if not surface_id:
        return {"error": "surface_id required"}
    ...
    parent   = surface.get("parent_dataset", "") or "discovered"
    crawl_id = surface.get("crawl_id", "") or ""
    tags     = ["discovered", "api", kind]

    # Count api_endpoints rows already stored under this dataset so we can report
    # exactly how many endpoints THIS enumeration added.
    def _ep_rows() -> int:
        try:
            return _sqlite_conn().execute(
                "SELECT COALESCE(SUM(row_count),0) AS n FROM fabric_subtables "
                "WHERE parent_dataset=? AND kind='api_endpoints'", (parent,)
            ).fetchone()["n"] or 0
        except Exception:
            return 0

    before = _ep_rows()
    budget = [12]   # generous probe budget for an explicit, user-triggered call
    try:
        if kind == "openapi":
            await _maybe_openapi_subtable(surface, parent, crawl_id, tags, budget)
        elif kind in ("json_api", "json"):
            await _maybe_api_enumeration(surface, parent, crawl_id, tags, budget)
        elif kind == "graphql":
            return {"error": "GraphQL introspection is not enumerable here — use "
                             "promote to register it as a queryable source"}
        else:
            return {"error": f"surface kind '{kind}' is not an enumerable API "
                             f"(expected openapi / json_api / json)"}
    except Exception as e:
        return {"error": f"enumeration failed: {e}"}

    added = max(0, _ep_rows() - before)
    result: Dict[str, Any] = {"ok": True, "endpoints": added,
                              "parent_dataset": parent, "kind": kind}
    if added == 0:
        result["note"] = ("no endpoints enumerated — the API may require auth or "
                          "expose no discoverable index/spec")

    # Optionally turn the surface into a recurring source (connector) too.
    if promote:
        pr = await promote_surface(surface, auto_pull=auto_pull)
        if pr.get("ok"):
            result["source_id"] = pr.get("source_id")
            result["dataset_id"] = pr.get("dataset_id")
            result["pulled"] = pr.get("pulled")
        else:
            result["promote_error"] = pr.get("error")

    await emit_event({"type": "fabric.discover.enumerated",
                      "surface_id": surface_id, "kind": kind,
                      "endpoints": added, "parent_dataset": parent,
                      "source_id": result.get("source_id")})
    return result


# ─────────────────────────────────────────────────────────────────────────────
# API BROWSER — list discovered APIs and map their data shape
# ─────────────────────────────────────────────────────────────────────────────
...
@capability(
    "fabric.api.list",
    http_method="GET", http_path="/fabric/api/list",
    http_tags=["fabric", "discover", "api"], memory="off", silent=True,
    description="List discovered API surfaces across all datasets for the API "
                "browser. Each API is annotated with its enumerated endpoint "
                "sub-tables and whether it has been promoted to a recurring source. "
                "Input: dataset_id (str — optional filter), include_endpoints "
                "(bool=True), limit (int=300). "
                "Output: {apis:[{id,url,label,kind,source_type,parent_dataset,"
                "promoted,source_id,confidence,topic_score,config,endpoint_tables:"
                "[{sub_dataset,title,rows}],endpoint_count}], count}.",
)
async def cap_api_list(dataset_id: str = "", include_endpoints: bool = True,
                       limit: int = 300, trace_id=None) -> Dict:
    _ensure_tables()
    if isinstance(include_endpoints, str):
        include_endpoints = include_endpoints.lower() in ("1", "true", "yes", "on")
    conn = _sqlite_conn()
    where = ["(source_type='api' OR kind IN ('openapi','json_api','json','graphql'))"]
    params: List[Any] = []
    if dataset_id:
        where.append("parent_dataset = ?"); params.append(dataset_id)
    sql = ("SELECT * FROM fabric_surfaces WHERE " + " AND ".join(where) +
           " ORDER BY topic_score DESC, confidence DESC LIMIT ?")
    params.append(min(int(limit), 1000))
    try:
    ...
    # Pre-load api_endpoints sub-tables per dataset once (avoids N queries).
    ep_by_ds: Dict[str, List[Dict]] = {}
    if include_endpoints:
        try:
            for e in conn.execute(
                "SELECT parent_dataset, sub_dataset, title, row_count "
                "FROM fabric_subtables WHERE kind='api_endpoints' "
                "ORDER BY row_count DESC LIMIT 2000"
            ).fetchall():
                ep_by_ds.setdefault(e["parent_dataset"], []).append(
                    {"sub_dataset": e["sub_dataset"], "title": e["title"],
                     "rows": e["row_count"]})
        except Exception:
            pass
    apis = []
    for r in rows:
        ...
        try:
            ...
            cfg = {}
        eps = ep_by_ds.get(d.get("parent_dataset"), []) if include_endpoints else []
        apis.append({
            "id": d.get("id"), "url": d.get("url"),
            "label": d.get("label") or d.get("url"),
            "kind": d.get("kind"), "source_type": d.get("source_type"),
            "parent_dataset": d.get("parent_dataset"),
            "promoted": bool(d.get("promoted")), "source_id": d.get("source_id") or "",
            "confidence": d.get("confidence"), "topic_score": d.get("topic_score"),
            "config": cfg, "endpoint_tables": eps,
            "endpoint_count": sum((e["rows"] or 0) for e in eps),
        })
...
@capability(
    "fabric.api.map",
    http_method="POST", http_path="/fabric/api/map",
    http_tags=["fabric", "discover", "api"], memory="off",
    description="Sample an API endpoint and MAP its response shape: locate the "
                "record array, infer a flat field schema, return a small sample, "
                "and suggest a jq_path so it can be wired up as a pull source. "
                "Input: surface_id (str) OR url (str), headers (JSON str='{}'), "
                "sample (int=5). "
                "Output: {ok, url, root_type, count, suggested_jq, "
                "schema:[{field,type,example}], records:[...]}.",
)
async def cap_api_map(surface_id: str = "", url: str = "", headers: str = "{}",
                      sample: int = 5, trace_id=None) -> Dict:
    _ensure_tables()
    if surface_id and not url:
        row = _sqlite_conn().execute(
            "SELECT url FROM fabric_surfaces WHERE id=?", (surface_id,)).fetchone()
        if row:
            url = row["url"]
    ...
    # Locate the record array and the jq_path that reaches it.
    suggested_jq, arr = "", None
    if isinstance(data, list):
        arr = data
    elif isinstance(data, dict):
        for k in ("results", "items", "data", "records", "rows", "entries", "value"):
            if isinstance(data.get(k), list):
                arr, suggested_jq = data[k], k
                break
        if arr is None:
            for k, v in data.items():
                if isinstance(v, list) and v:
                    arr, suggested_jq = v, k
                    break
        if arr is None:
            arr = [data]
    else:
    ...
    root_type = ("array" if isinstance(data, list)
                 else "object" if isinstance(data, dict) else "scalar")
    schema_map: Dict[str, Dict] = {}
    for it in (arr or [])[:20]:
        if isinstance(it, dict):
            for k, v in it.items():
                if k not in schema_map:
                    schema_map[k] = {"field": k, "type": _json_type(v),
                                     "example": _short_ex(v)}
        else:
            schema_map.setdefault("(value)", {"field": "(value)",
                                              "type": _json_type(it),
                                              "example": _short_ex(it)})
    return {"ok": True, "url": url, "root_type": root_type,
            "count": len(arr or []), "suggested_jq": suggested_jq,
            "schema": list(schema_map.values()),
            "records": (arr or [])[:max(1, min(int(sample or 5), 25))]}
...
async def cap_discover_delete_scan(crawl_id: str, delete_dataset: bool = False,
    ...
    if delete_dataset and ds_id:
        ...
        try:
            from Vera.vera.fabric.data_fabric import fabric_dataset_delete
...
async def cap_discover_graph(
    ...
    for e in erows:
        ...
        edges.append({"from": f, "to": t, "rel": rel, "layer": "discover"})
    ...
    if include_entities and ds_id:
        # record_id → page url/title so MENTIONS attaches to the SOURCE page node
        # (the node the entity was extracted from) rather than the dataset hub.
        # Seed from the labelled page nodes, then supplement with EVERY record in
        # the dataset — many pages (seeds, leaf pages, YouTube/API records) carry
        # entities but have no discovery edge, so they're absent from `labels`;
        # without this they'd all collapse onto the central dataset node.
        rec2url: Dict[str, str] = {}
        rec2title: Dict[str, str] = {}
        ...
        for m in erow:
            ...
            if purl:
                # Materialise the source page node if it isn't already on the
                # graph, so the entity links to the page it came from.
                if purl not in nodes:
                    nodes[purl] = {"id": purl,
                                   "label": (rec2title.get(m["record_id"]) or _short(purl))[:42],
                                   "type": "Page",
                                   "props": {"id": purl, "url": purl,
                                             "title": rec2title.get(m["record_id"], "")}}
                    if ds_id in nodes:
                        edges.append({"from": ds_id, "to": purl, "rel": "HAS_PAGE", "layer": "discover"})
                edges.append({"from": purl, "to": eid, "rel": "MENTIONS", "layer": "entities"})
            elif ds_id in nodes:
                edges.append({"from": ds_id, "to": eid, "rel": "MENTIONS", "layer": "entities"})
    ...
    # Cap node count — give entities their own budget so high-degree structural
    # nodes don't crowd them out entirely.
    ...
    if len(node_list) > max_nodes:
        ...
        structural = [n for n in node_list if n["type"] != "Entity"]
        entity_nodes = [n for n in node_list if n["type"] == "Entity"]
        entity_budget = max(max_nodes // 2, max_nodes - len(structural))
        struct_budget = max_nodes - min(len(entity_nodes), entity_budget)
        structural.sort(key=lambda n: (n["props"].get("root", False), deg.get(n["id"], 0)),
                        reverse=True)
        entity_nodes.sort(key=lambda n: deg.get(n["id"], 0), reverse=True)
        node_list = structural[:struct_budget] + entity_nodes[:entity_budget]
...
async def _llm_expand_queries(topic: str, n: int, goal: str = "",
                              brief: Dict = None, description: str = "") -> List[str]:
    """Ask the LLM for high-signal search queries / subtopics for the topic.

    Builds on a grounding `description` of the topic (see _describe_topic) so the
    angles aren't generic template variations. When a GOAL (and/or research
    brief) is supplied the queries are tailored to surface the data the goal
    needs — e.g. topic 'pokemon gold/silver pokedex' + goal 'create a usable
    pokedex' yields queries for stats, types, evolutions, locations rather than
    ...
    prompt = f"Topic: {topic}\n"
    if description:
        prompt += f"What the topic is: {description}\n"
    if goal:
        prompt += (f"GOAL — what the user wants to BUILD from the crawl: {goal}\n"
                   f"Bias every query toward finding the concrete data/pages needed "
                   f"to achieve this goal.\n")
    if brief:
        props = ", ".join((brief.get("properties") or [])[:8])
        must  = ", ".join((brief.get("must_terms") or [])[:8])
        if props:
            prompt += f"Data fields to capture: {props}\n"
        if must:
            prompt += f"Distinctive terms to favour: {must}\n"
    prompt += (f"\nGive {n} diverse, specific web-search queries that together "
               f"cover the most relevant facets, key entities, and authoritative "
               f"sources"
               + (" for achieving the goal" if goal else " for this topic")
               + ". One query per line.")
...
async def _describe_topic(topic: str, goal: str = "") -> Dict:
    """FIRST step of discovery: get a short grounding description of the topic so
    every later step (query angles, brief, relevance) builds on a shared base
    instead of prescriptive query templates.

    If the LLM does NOT recognise the term, run a plain web search first and write
    the description FROM those results — so obscure / novel terms still get a
    solid footing before the real crawl begins.
    Returns {description, known, grounded}."""
    sys = ("You write a 1-3 sentence factual description of a topic to ground a "
           "research crawl. If you do NOT recognise the topic, or are unsure what "
           "it refers to, reply with EXACTLY 'UNKNOWN' and nothing else.")
    pr = "Topic: " + topic + "\n"
    if goal:
        pr += "Goal the user wants to achieve: " + goal + "\n"
    pr += "Describe what this topic is (or reply UNKNOWN)."
    desc = (await _llm_generate(pr, sys, timeout=40) or "").strip()
    known = bool(desc) and "unknown" not in desc.lower()[:14]
    grounded = False
    if not known:
        # The model doesn't know the term — learn it from a plain web search,
        # then describe from the actual results.
        try:
            hits = await _web_search(topic, 6)
        except Exception:
            hits = []
        snippets = "\n".join(
            ("- " + (h.get("title") or "") + ": " + (h.get("content") or "")[:300])
            for h in (hits or [])[:6])[:3000]
        if snippets:
            sys2 = ("Using ONLY the supplied search snippets, write a 1-3 sentence "
                    "factual description of what this topic refers to. Be concrete; "
                    "do not speculate beyond the snippets.")
            pr2 = ("Topic: " + topic + "\n"
                   + (("Goal: " + goal + "\n") if goal else "")
                   + "Search results:\n" + snippets + "\n\nDescription:")
            d2 = (await _llm_generate(pr2, sys2, timeout=40) or "").strip()
            if d2:
                desc, grounded = d2, True
        if not desc or not grounded:
            desc = desc if (desc and known) else topic   # last-resort fallback
    return {"description": desc[:600], "known": known, "grounded": grounded}


async def _llm_steer_crawl(topic: str, ds_id: str, n_angles: int = 6,
                           searched: Optional[Set[str]] = None,
                           goal: str = "") -> List[str]:
    """Analyse crawl progress and return new search queries targeting coverage gaps.

    Reads recent pages + top entities from the dataset, then asks the LLM which
    important angles are missing. Returns up to n_angles fresh query strings.
    """
    searched = searched or set()
    try:
        rows = _sqlite_conn().execute(
            "SELECT title, description FROM fabric_records "
            "WHERE dataset_id=? AND title IS NOT NULL "
            "ORDER BY ROWID DESC LIMIT 25",
            (ds_id,),
        ).fetchall()
        summaries = [
            f"{r['title']}: {(r['description'] or '')[:120]}".strip()
            for r in rows if r["title"]
        ]
    except Exception:
        summaries = []
    try:
        ent_rows = _sqlite_conn().execute(
            "SELECT name, type FROM fabric_entities "
            "WHERE datasets LIKE ? ORDER BY mention_count DESC LIMIT 25",
            ('%"' + ds_id + '"%',),
        ).fetchall()
        entities = [f"{r['name']} ({r['type']})" for r in ent_rows]
    except Exception:
        entities = []
    ...
    coverage = ""
    if summaries:
        coverage += "Pages crawled so far:\n" + "\n".join(f"- {s}" for s in summaries[:15])
    if entities:
        coverage += "\n\nEntities found:\n" + ", ".join(entities[:15])
    if searched:
        coverage += "\n\nAlready searched: " + ", ".join(list(searched)[:20])
    sys_prompt = (
        "You are a research director steering an autonomous web crawl. "
        "Analyse what has been found and identify the most important coverage gaps. "
        "Output ONLY search queries, one per line, no numbering, no commentary."
    )
    user_prompt = (
        f"Topic: {topic}\n\n"
        + (f"GOAL — what the user is trying to BUILD: {goal}\n"
           f"Prioritise gaps in the data needed to achieve this goal.\n\n" if goal else "")
        + f"{coverage}\n\n"
        f"Generate {n_angles} search queries that address the most important "
        f"gaps or unexplored angles not already covered above. "
        f"Be specific and targeted — avoid repeating what was already searched."
    )
    txt = await _llm_generate(user_prompt, sys_prompt, timeout=35.0)
    raw = _parse_lines(txt, n_angles * 2)
    # Filter out queries too similar to already-searched terms
    out = []
    for q in raw:
...
async def _llm_structure(text: str, content_type: str = "web") -> Dict:
    ...
    try:
        ...
        return {"entities": [], "relations": []}
    return _parse_struct_obj(obj)


def _parse_struct_obj(obj) -> Dict:
    """Normalise an LLM structure object {entities[], relations[]} into the
    canonical {entities, relations} shape — typed/de-duped entities and
    relation endpoints guaranteed to reference returned entities. Shared by
    _llm_structure (entities-only call) and _llm_page_analyze (combined call)."""
...
async def _llm_page_analyze(text: str, title: str, topic: str,
                            content_type: str = "web") -> Dict:
    """ONE LLM round-trip that does what _llm_tag + _llm_structure did in two:
    rate on-topic relevance, tag the page, AND extract the typed entity/relation
    structure. Returns {relevance: float|None, tags: [str], entities: [...],
    relations: [...]} using the SAME entity/relation shape as _llm_structure so
    it drops straight into the persistence path. Degrades to {} on failure so
    callers can fall back to the separate calls."""
    snippet = (text or "")[:5000].strip()
    if not snippet:
        return {}
    sys = (
        "You are an information-extraction engine. In ONE pass over a web page "
        "you (a) judge how on-topic it is for the given TOPIC, (b) tag it, and "
        "(c) capture the STRUCTURE of its content: the key entities and how they "
        "relate. Respond with ONLY a JSON object of the form "
        '{"relevance":number,"tags":[str],'
        '"entities":[{"name":str,"type":str,"salience":number}],'
        '"relations":[{"from":str,"to":str,"rel":str}]}. '
        "relevance is 0..1 (how on-topic the page is for TOPIC). tags: up to 8 "
        "short topical labels. "
        "Allowed entity types: person, organisation, product, technology, "
        "concept, standard, protocol, location, event, date, role, work. "
        "RULES: (1) Use 'person' ONLY for an individual human being. (2) Prefer "
        "fewer, high-value entities the topic is actually about. (3) salience is "
        "0..1. (4) Every relation 'from'/'to' MUST be a name in your entities "
        "list. (5) Use SPECIFIC UPPER_SNAKE verbs (CREATED_BY, DEVELOPED_BY, "
        "PART_OF, USES, BASED_ON, SUCCEEDS, REPLACES, COMPETES_WITH, WORKS_FOR, "
        "FOUNDED, MEMBER_OF, LOCATED_IN, OCCURRED_IN, PUBLISHED_BY, INFLUENCED, "
        "OWNS, LEADS, ACQUIRED_BY, IMPLEMENTS, EXTENDS, DEFEATS, APPEARED_IN). "
        "(6) NEVER use RELATED_TO or CO_OCCURS — omit the relation if no specific "
        "verb fits. Quality over quantity."
    )
    head = (title or "").strip()
    prompt = (f"TOPIC: {topic}\n\nPAGE TITLE: {head}\n\nTEXT:\n{snippet}\n\nJSON:")
    txt = await _llm_generate(prompt, sys, timeout=45.0)
    if not txt:
        return {}
    m = re.search(r"\{.*\}", txt, re.S)
    if not m:
    ...
    if not isinstance(obj, dict):
        return {}
    out = _parse_struct_obj(obj)   # entities + relations, validated/typed
    if isinstance(obj.get("relevance"), (int, float)):
        out["relevance"] = max(0.0, min(1.0, float(obj["relevance"])))
    if isinstance(obj.get("tags"), list):
        out["tags"] = [str(t)[:40] for t in obj["tags"][:8] if str(t).strip()]
    return out
...
async def _web_search(query: str, limit: int = 8) -> List[Dict]:
    ...
    try:
        import Vera.vera.research.researcher_api as ra
    except Exception:
        return []
...
async def cap_discover_topic(
    ...
    page_text_cap: int = 0, page_link_cap: int = 0, max_record_chars: int = 0,
    llm_steering: bool = False, steering_interval: int = 60, goal: str = "",
    user_agent: str = "", ua_rotate: bool = False,
    trace_id=None,
) -> Dict:
    ...
    # Grounding description is folded into `config` once it's built below; keep it
    # defined for both seed paths so that reference can't UnboundLocalError.
    topic_desc = ""
    if explicit:
        seeds = explicit
    else:
        # ── Step 1: ground the topic with a short LLM description ─────────────
        # Everything downstream (angles, brief, relevance) builds on this base
        # instead of prescriptive query templates. Unrecognised terms get a plain
        # web search first so the description is still solid (see _describe_topic).
        if llm_search:
            await _emit("describing", message=f"grounding topic '{topic}'…")
            try:
                _di = await _describe_topic(topic, goal)
            except Exception as e:
                _di = {}; log.debug("describe topic: %s", e)
            topic_desc = (_di or {}).get("description", "") or ""
            if topic_desc:
                await _emit("described", description=topic_desc,
                            grounded=bool((_di or {}).get("grounded")),
                            known=bool((_di or {}).get("known", True)),
                            message=("grounded via web search: "
                                     if (_di or {}).get("grounded")
                                     else "topic: ") + topic_desc[:160])
        # ── Step 2: derive angles FROM the description (LLM-led, not a fixed
        # template list). The static templates are only a fallback so a search
        # still runs if the LLM is unavailable. ──────────────────────────────
        queries = []
        if llm_search:
            try:
                await _emit("llm_expanding",
                            message="LLM generating search angles from the topic description"
                                    + (f" toward goal: {goal[:50]}" if goal else "") + "…")
                queries = await _llm_expand_queries(topic, max(4, search_angles),
                                                    goal=goal, description=topic_desc)
            except Exception as e:
                log.debug("llm angles: %s", e)
        if len(queries) < 2:
            queries = (queries or []) + _topic_query_angles(topic, search_angles)
        # de-dupe, preserve order
        _seen_q: Set[str] = set()
        queries = [q for q in queries
                   if q and not (q.lower() in _seen_q or _seen_q.add(q.lower()))]
        await _emit("angles_generated",
                    message=f"{len(queries)} search angles for '{topic}'",
                    queries=queries[:10])
        for _qi, _q in enumerate(queries[:search_angles]):
            await _emit("angle_query", index=_qi + 1, total=len(queries),
                        query=_q,
                        message=f"angle {_qi+1}/{len(queries)}: {_q!r}")
        ...
        await _emit("seeded_web", found=len(discovered),
                    message=f"web search: {len(discovered)} seed URLs from {len(queries)} angles")
        ...
        if site_qs:
            queries += site_qs
            await _emit("site_angles",
                        sites=sites, count=len(site_qs),
                        queries=site_qs[:6],
                        message=f"site-targeted angles ({sites}): "
                                + ", ".join(repr(x) for x in site_qs[:4]))
            try:
                _site_found = await _search_many(site_qs, results_per_search)
                discovered += _site_found
                await _emit("site_seeded", found=len(_site_found),
                            message=f"site search: {len(_site_found)} additional seeds")
            except Exception as e:
    ...
    config = {
        ...
        "topic_brief": topic_brief, "brief_rounds": brief_rounds,
        "goal": goal.strip(),
        "llm_steering": llm_steering, "steering_interval": steering_interval,
        "user_agent": user_agent.strip(), "ua_rotate": ua_rotate,
        "llm_tagging": llm_tagging, "min_relevance": min_relevance,
    ...
    await _emit("seeded", message=f"{len(seeds)} seeds from {len(queries)} queries",
                seeds=len(seeds))
    queue = [(-1.0, i, u, 0, 0, None) for i, u in enumerate(seeds)]
    ...
    for rnd in range(max(0, expansion_rounds)):
        ...
        await _emit("expanding", round=rnd + 1, concepts=concepts, queries=cqueries,
                    message=f"expansion round {rnd+1}: top concepts → "
                            + ", ".join(repr(c) for c in concepts[:5]))
        for _qi, _q in enumerate(cqueries):
            await _emit("angle_query", round=rnd + 1, index=_qi + 1, total=len(cqueries),
                        query=_q,
                        message=f"  round {rnd+1} angle {_qi+1}: {_q!r}")
        found = await _search_many(cqueries, results_per_search)
        await _emit("expansion_found", round=rnd + 1, found=len(found),
                    message=f"round {rnd+1}: {len(found)} new seed URLs found")
        if not found:
            await _emit("expansion_empty", round=rnd + 1,
                        message=f"round {rnd+1}: no new seeds — stopping expansion")
            continue
        ...
        if not fr:
            break
        try:
            _raw_q2 = json.loads(fr.get("queue") or "[]")
            if _raw_q2 and isinstance(_raw_q2[0][0], str):
                q2 = [(-max(0.0, 1.0 - 0.2 * x[1]), i, x[0], x[1], x[2],
                        x[3] if len(x) > 3 else None)
                       for i, x in enumerate(_raw_q2)]
            else:
                q2 = [tuple(x) for x in _raw_q2]
            vis = set(json.loads(fr.get("visited") or "[]"))
        except Exception:
            q2, vis = [], set()
        new_seeds = [d["url"] for d in found if d["url"] not in vis]
        await _emit("new_seeds_found", round=rnd + 1, count=len(new_seeds),
                    urls=new_seeds[:5],
                    message=f"round {rnd+1}: {len(new_seeds)} new URLs queued for crawl"
                            + (f": {new_seeds[0][:80]}…" if new_seeds else ""))
        for i, u in enumerate(new_seeds):
            heapq.heappush(q2, (-1.0, i, u, 0, 0, None))
        if not new_seeds:
            continue
        cfg2 = dict(config)
    ...
    # ── 3b) LLM steering rounds — analyse coverage gaps and inject new angles ─
    steering_rounds_run = 0
    if llm_steering and expansion_rounds > 0:
        _max_steer_rounds = max(1, expansion_rounds)
        for _sr in range(_max_steer_rounds):
            await _emit("steering_start", round=_sr + 1,
                        message=f"LLM steering round {_sr+1}: analysing crawl coverage…")
            steer_angles = await _llm_steer_crawl(
                topic, ds_id, n_angles=searches_per_round,
                searched=searched_terms, goal=goal,
            )
            if not steer_angles:
                await _emit("steering_empty", round=_sr + 1,
                            message=f"steering round {_sr+1}: LLM found no new gaps — done")
                break
            await _emit("steering_angles", round=_sr + 1,
                        angles=steer_angles,
                        message=f"steering round {_sr+1}: LLM identified {len(steer_angles)} gap queries: "
                                + ", ".join(repr(a) for a in steer_angles[:4]))
            for _qi, _q in enumerate(steer_angles):
                await _emit("angle_query", kind="steering", round=_sr + 1,
                            index=_qi + 1, total=len(steer_angles), query=_q,
                            message=f"  steering {_sr+1} angle {_qi+1}: {_q!r}")
                searched_terms.add(_q.lower())
            found_s = await _search_many(steer_angles, results_per_search)
            await _emit("steering_found", round=_sr + 1, found=len(found_s),
                        message=f"steering round {_sr+1}: {len(found_s)} URLs from gap queries")
            if not found_s:
                continue
            fr_s = _load_frontier(cid)
            if not fr_s:
                break
            try:
                _raw_qs = json.loads(fr_s.get("queue") or "[]")
                if _raw_qs and isinstance(_raw_qs[0][0], str):
                    qs2 = [(-max(0.0, 1.0 - 0.2 * x[1]), i, x[0], x[1], x[2],
                             x[3] if len(x) > 3 else None)
                            for i, x in enumerate(_raw_qs)]
                else:
                    qs2 = [tuple(x) for x in _raw_qs]
                vis_s = set(json.loads(fr_s.get("visited") or "[]"))
            except Exception:
                qs2, vis_s = [], set()
            new_s = [d["url"] for d in found_s if d["url"] not in vis_s]
            await _emit("steering_queued", round=_sr + 1, count=len(new_s),
                        urls=new_s[:5],
                        message=f"steering round {_sr+1}: {len(new_s)} new gap URLs queued")
            for i, u in enumerate(new_s):
                heapq.heappush(qs2, (-1.0, i, u, 0, 0, None))
            if not new_s:
                continue
            cfg_s = dict(config)
            cfg_s["max_pages"] = max_pages + rounds_run * max(20, results_per_search * searches_per_round)
            await _emit("steering_crawl", round=_sr + 1,
                        message=f"steering round {_sr+1}: crawling {len(new_s)} gap URLs…")
            result = await _run_crawl(cid, ds_id, seeds[0], cfg_s, qs2, vis_s)
            steering_rounds_run += 1
...
async def cap_discover_map_topic(
    ...
    sites: str = "reddit,x,youtube,news", use_llm_entities: bool = True,
    llm_steering: bool = False, focus: str = "", crawl_id: str = "",
    search_angles: int = 0,
    negative_keywords: str = "", exclude_urls: str = "",
    required_keyword: str = "", required_keyword_mode: str = "fuzzy",
    ...
    # Allow the caller to override the preset search_angles value
    if search_angles and search_angles > 0:
        cfg = dict(cfg, search_angles=search_angles)
    _eff_sites = sites or "reddit,x,youtube,news"
    try:
        await emit_event({"type": "fabric.discover.progress", "stage": "map_start",
                          "engine": "discovery",
                          "message": f"Mapping topic '{topic.strip()}' "
                                     f"(depth={depth}, sites={_eff_sites}, "
                                     f"angles={cfg.get('search_angles',6)})"})
    except Exception:
        pass
    return await cap_discover_topic(
        topic=topic, dataset_id=dataset_id, sites=_eff_sites,
        ...
        negative_words=negative_keywords, negative_urls=exclude_urls,
        required_keyword=required_keyword, required_keyword_mode=required_keyword_mode,
        max_concurrency=max_concurrency, entity_workers=entity_workers,
...
async def _gen_brief(topic: str, samples: str = "", prev: Dict = None,
                     goal: str = "") -> Dict:
    ...
    avoid_terms[], goal, plan[]} — must_terms boost relevance, avoid_terms flag
    sibling drift. When a GOAL is given the brief is oriented toward producing
    what the goal needs (e.g. topic 'pokemon gold/silver pokedex' + goal 'create
    a usable pokedex' → properties become the fields a pokedex entry needs)."""
    ...
    pr = "TOPIC: " + topic + "\n"
    if goal:
        pr += ("GOAL (what the user wants to BUILD from this crawl): " + goal + "\n"
               "Orient the brief toward this goal: 'properties' should be the "
               "concrete fields/attributes needed to fulfil the goal, and "
               "'must_terms' should favour pages that carry that data.\n")
    if prev:
    ...
    pr += ('\nReturn ONLY {"type":"what kind of thing the topic is",'
           ...
           + (',"goal":"restate the user goal in one sharp sentence",'
              '"plan":["ordered steps / data to gather to achieve the goal"]'
              if goal else '')
           + ',"properties":["key attributes worth '
    ...
    if not isinstance(d, dict):
        return {}
    for k in ("properties", "must_terms", "avoid_terms", "plan"):
        v = d.get(k)
    ...
    if goal:
        d["goal"] = str(d.get("goal", "") or goal)[:400]
    return d
...
async def cap_synthesize_topic(
    ...
    if _prof_fn and entities and records:
        ...
        await _emit("profiling", current=0, total=len(_top_ents),
                    message=f"building entity profiles for {len(_top_ents)} top entities…")
        _profiled = 0
        for _idx, _e in enumerate(_top_ents):
            _name = _e.get("name", "")
            if not _name or _e.get("description"):  # skip if already profiled
                continue
            # Heartbeat so the terminal isn't silent during the per-entity LLM
            # calls (each can take a few seconds).
            await _emit("profiling", current=_idx + 1, total=len(_top_ents),
                        entity=_name[:40],
                        message=f"profiling {_idx+1}/{len(_top_ents)}: {_name[:40]}")
            _ctxs = []
    ...
    # Nothing to synthesise from — don't silently persist an empty model.
    # Report WHY so the caller/UI can tell scope-miss from LLM/plan failure.
    if not targets:
        if not datasets:
            reason = (f"no dataset resolved for topic '{topic}' — pass dataset_id "
                      f"explicitly, or run a discovery crawl for it first")
        elif not entities and not records:
            reason = (f"dataset(s) {datasets} have no entities or records in scope "
                      f"— run/finish a discovery crawl with entity extraction first")
        else:
            reason = ("the planner produced no enumerable entries and no salient "
                      "entities were found to distil")
        await _emit("empty", datasets=datasets, entities=len(entities),
                    records=len(records),
                    message=f"nothing to synthesise: {reason}")
        return {"ok": False, "error": reason, "topic": topic,
                "entry_type": entry_type, "entries": [], "entry_count": 0,
                "relations": [], "datasets": datasets, "gaps": gaps,
    ...
    for bi in range(0, len(targets), BATCH):
        ...
        syn_prompt = (
            ...
            + (f"FOCUS: {focus}\n" if focus else "")
            + f"Distil each item below into one factual entry that explains its place "
              f"in the bigger picture of '{topic}'. Return ONLY:\n"
            '{"entries":[{"name":"","type":"","description":"3-4 sentences: what this '
            'item IS, grounded in the sources, AND how it relates to / what role it '
            'plays in the topic as a whole","attributes":{},'
    ...
    # The distillation LLM returned nothing usable for any batch — surface it
    # rather than persisting/emitting "done" on an empty model.
    if not entries:
        reason = (f"distillation produced no entries from {len(targets)} candidate "
                  f"items (LLM returned no usable JSON, or no grounded evidence)")
        await _emit("empty", targets=len(targets),
                    message=f"nothing to synthesise: {reason}")
        return {"ok": False, "error": reason, "topic": topic,
                "entry_type": entry_type, "entries": [], "entry_count": 0,
                "relations": [], "datasets": datasets, "gaps": gaps,
            ...
            try:
                await g.upsert_node("TopicModel", model_id,
                                    {"id": model_id, "topic": topic,
                                     ...
                                     "dataset_id": ds_primary, "layer": "3rd_order"})
                for e in entries:
                ...
                for r in flat_relations[:500]:
                    await g.query(
                        ...
                        "SET x.why=$why, x.kind=$kind, x.layer='3rd_order'",
...
async def cap_discover_expand(
    ...
    if nl == "page" and mode == "synthesize":
        ...
        try:
            ...
            if not r.get("ok"):
                return {"error": r.get("error", "synthesis produced no model")}
            n_entries = r.get("entry_count", len(r.get("entries", []) or []))
            return {"ok": True, "nodes": [], "edges": [],
                    "added": 0,
                    "topic": r.get("topic", topic_for_synth),
                    "model_id": r.get("model_id", ""),
                    "entries": n_entries,
                    "note": f"3rd-order model built for {ds} ({n_entries} entries)."}
        except Exception as e:
    ...
    if nl == "page":
        ...
        try:
            await _run_crawl(sub_cid, ds, url, config, [(-1.0, 0, url, 0, 0, None)], set())
        except Exception as e:
    ...
    # ── Entity: expand to neighbouring entities + the records that mention it ─
    if nl == "entity":
        conn = _sqlite_conn()
        eid = node_id
        nodes, edges, seen = [], [], set()
        def _add(nid, label, ntype, props=None):
            if nid and nid not in seen:
                seen.add(nid)
                nodes.append({"id": nid, "label": str(label or nid)[:42],
                              "type": ntype, "props": props or {"id": nid}})
        try:
            er = conn.execute("SELECT id,name,type,mention_count FROM fabric_entities "
                              "WHERE id=?", (eid,)).fetchone()
        except Exception:
            er = None
        if er:
            _add(er["id"], er["name"] or er["type"], "Entity",
                 {"id": er["id"], "name": er["name"], "type": er["type"],
                  "mention_count": er["mention_count"]})
        # neighbouring entities via the entity-relation table
        try:
            rels = conn.execute("SELECT from_id,to_id,rel FROM fabric_entity_relations "
                                "WHERE from_id=? OR to_id=? LIMIT ?",
                                (eid, eid, max_links)).fetchall()
        except Exception:
            rels = []
        nbr_ids = {(r["to_id"] if r["from_id"] == eid else r["from_id"]) for r in rels}
        nbr_ids.discard(eid)
        if nbr_ids:
            ph = ",".join("?" for _ in nbr_ids)
            try:
                for nr in conn.execute(
                    "SELECT id,name,type,mention_count FROM fabric_entities "
                    "WHERE id IN (%s)" % ph, tuple(nbr_ids)).fetchall():
                    _add(nr["id"], nr["name"] or nr["type"], "Entity",
                         {"id": nr["id"], "name": nr["name"], "type": nr["type"],
                          "mention_count": nr["mention_count"]})
            except Exception:
                pass
        for r in rels:
            if r["from_id"] in seen and r["to_id"] in seen:
                edges.append({"from": r["from_id"], "to": r["to_id"],
                              "rel": r["rel"] or "RELATED_TO"})
        # source records that mention this entity → page / record nodes
        try:
            mrow = conn.execute("SELECT record_id FROM fabric_entity_mentions "
                                "WHERE entity_id=? LIMIT ?", (eid, max_links)).fetchall()
        except Exception:
            mrow = []
        rec_ids = [m["record_id"] for m in mrow if m["record_id"]]
        if rec_ids:
            ph = ",".join("?" for _ in rec_ids)
            try:
                for rr in conn.execute(
                    "SELECT data FROM fabric_records WHERE id IN (%s) LIMIT %d"
                    % (ph, max_links), tuple(rec_ids)).fetchall():
                    try:
                        d = json.loads(rr["data"]) if rr["data"] else {}
                    except Exception:
                        continue
                    u = _normalize_url(d.get("url") or "")
                    nid = u or d.get("id")
                    if nid:
                        _add(nid, d.get("title") or u or nid,
                             "Page" if u else "FabricRecord",
                             {"id": nid, "url": u, "title": d.get("title", "")})
                        edges.append({"from": nid, "to": eid, "rel": "MENTIONS"})
            except Exception:
                pass
        return {"ok": True, "nodes": nodes, "edges": edges, "added": len(nodes),
                "note": f"Expanded entity → {len(nbr_ids)} related entities, "
                        f"{len(rec_ids)} source records."}
...
def _register_node_actions():
    ...
    _NODE_ACTION_REGISTRY["Surface"] = [
        ...
        {"id": "enumerate_api", "label": "Enumerate API endpoints", "icon": "\u2630",
         "capability": "fabric.surfaces.enumerate", "args": {"surface_id": "$id"},
         "options": [{"name": "promote", "type": "bool", "default": False,
                      "label": "Also create connector"},
                     {"name": "auto_pull", "type": "bool", "default": False,
                      "label": "Pull connector now"}],
         "context": "For an OpenAPI/Swagger spec or REST resource-index, walk the "
                    "whole API and capture every endpoint into an api_endpoints "
                    "sub-table. Optionally register it as a recurring connector."},
        {"id": "map_api", "label": "Map API data", "icon": "\u25eb",
         "capability": "fabric.api.map", "args": {"surface_id": "$id"},
         "options": [{"name": "sample", "type": "int", "default": 5,
                      "label": "Sample rows"}],
         "context": "Sample this API and infer its response shape \u2014 field schema, "
                    "a data sample, and a suggested jq_path for setting it up as a "
                    "pull source. See the API panel for the full browser."},
    ...
    _NODE_ACTION_REGISTRY["Entity"] = [
        {"id": "expand", "label": "Expand: related & sources", "icon": "\u2935",
         "capability": "fabric.discover.expand",
         "args": {"node_id": "$id", "node_label": "Entity"},
         "options": [{"name": "max_links", "type": "int", "default": 25,
                      "label": "Max neighbours / records"}],
         "context": "Pull in the entities this one co-occurs/relates with and the "
                    "source pages that mention it."},
        {"id": "view_records", "label": "Records mentioning", "icon": "\u2630",
         "capability": "__local",
         "context": "Show the records that mention this entity in the table drawer."},
        {"id": "focus", "label": "Focus in graph", "icon": "\u25c9",
         "capability": "__local"},
    ]
    log.info("fabric_discovery: registered graph node actions "
             "(Surface/Page/Subtable/Dataset/Entity)")

_register_node_actions()


# ═══════════════════════════════════════════════════════════════════════════
# COLLECTION RECONSTRUCTION — faithful "3rd-order extraction"
# ───────────────────────────────────────────────────────────────────────────
# Takes a structured resource spread over MANY same-template detail pages
# (e.g. a Pokédex: one entry per page) and rebuilds it internally as ONE typed
# dataset + graph: every entry a node, a consistent field schema, and a
# DERIVED_FROM provenance edge back to its 1st-order source page record.
#
# This is lossless reconstruction — distinct from `fabric.synthesize.topic`,
# which is a lossy LLM distillation. The field-extraction strategy is a
# configurable knob (`field_map_mode`) so it scales from cheap heuristics to
# per-page LLM for messy layouts:
#   heuristic     — DOM key/value induction only, no LLM
#   template_llm  — LLM canonicalises the field map ONCE from samples, then a
#                   deterministic heuristic apply across every page (default)
#   per_page_llm  — LLM extracts a record from each page (flexible, costly)
#   auto          — heuristic if pages are regular, else per_page_llm
# ═══════════════════════════════════════════════════════════════════════════

_COLL_MODES = ("auto", "heuristic", "template_llm", "per_page_llm")


async def _coll_fetch(url: str, timeout: float = 25.0) -> Tuple[str, str]:
    """Fetch one page → (html, final_url). Returns ('', url) on failure."""
    try:
        import httpx
        ...
        log.debug("coll fetch %s: %s", url, e)
        return "", url


def _coll_key(s: str) -> str:
...
def _extract_kv(html: str) -> Dict[str, str]:
    """Pull a flat {field: value} dict out of an infobox-style detail page —
    2-column tables (th/td or td/td) and definition lists (dt/dd). This matches
    how most structured reference pages (Pokédex infoboxes, spec sheets, etc.)
    lay out their per-entry attributes."""
    soup = _soup(html)
    if not soup:
        return {}
    kv: Dict[str, str] = {}
    ...
    for table in soup.find_all("table"):
        for tr in table.find_all("tr"):
            ...
            if len(cells) != 2:
                continue
            k = _coll_key(cells[0].get_text(" ", strip=True))
            v = cells[1].get_text(" ", strip=True)
            if k and v and len(k) <= 50 and k not in kv:
                kv[k] = v[:500]
    # definition lists
    for dl in soup.find_all("dl"):
        dts = dl.find_all("dt")
        ...
        for dt, dd in zip(dts, dds):
            k = _coll_key(dt.get_text(" ", strip=True))
...
def _extract_detail_links(html: str, base_url: str, pattern: str = "") -> List[str]:
    """Candidate detail-page URLs from a list/index page: same-host anchors,
    deduped, optionally filtered by a regex `pattern` on the URL."""
    soup = _soup(html)
    if not soup:
        return []
    from urllib.parse import urljoin, urlparse
    ...
    for a in soup.find_all("a", href=True):
        ...
        if not full.startswith(("http://", "https://")):
            continue
        if (urlparse(full).hostname or "") != host:
            continue
        if pat and not pat.search(full):
            continue
...
async def _llm_field_map(samples: List[Dict[str, str]]) -> Dict[str, str]:
    """Ask the LLM to canonicalise raw kv keys across sample pages into a stable
    field map {raw_key: canonical_field}. Returns {} on failure (caller falls
    back to raw keys)."""
    if not samples:
        return {}
    ...
    if not keys:
        return {}
    sys = ("You normalise scraped record fields. Given raw field keys extracted "
           "from several same-template pages, map each to a clean, consistent "
           "snake_case canonical name. Drop noise/navigation keys by mapping them "
           "to empty string. Output strict JSON only.")
    pr = ("RAW KEYS:\n" + json.dumps(keys)
          + "\n\nSAMPLE RECORDS:\n" + json.dumps(samples[:3])[:3000]
          + '\n\nReturn ONLY {"map": {"raw_key": "canonical_name_or_empty", ...}}.')
    d = _sjson(await _llm_generate(pr, sys, timeout=60)) or {}
    m = d.get("map") if isinstance(d, dict) else None
    if not isinstance(m, dict):
        return {}
    return {str(k): str(v).strip() for k, v in m.items()}


async def _llm_record_from_page(html: str, fields: List[str]) -> Dict[str, str]:
    """per_page_llm extraction: ask the LLM for a JSON record over `fields`."""
    soup = _soup(html)
    text = soup.get_text(" ", strip=True)[:6000] if soup else html[:6000]
    sys = ("You extract one structured record from a page. Output strict JSON "
           "only, with exactly the requested keys; use empty string for missing.")
...
def _apply_field_map(kv: Dict[str, str], fmap: Dict[str, str]) -> Dict[str, str]:
    """Remap raw kv keys → canonical names per fmap; '' canonical drops the key.
    Unmapped keys pass through under their raw name."""
    if not fmap:
        return dict(kv)
    out: Dict[str, str] = {}
    for k, v in kv.items():
...
# ── fabric_collections persistence ───────────────────────────────────────────
def _coll_save(rec: Dict):
    _ensure_tables()
    try:
        conn = _sqlite_conn()
        conn.execute(
            "INSERT OR REPLACE INTO fabric_collections "
            "(id, name, root_url, dataset, entity_type, field_map, mode, "
            " detail_urls, done_urls, row_count, status, note, created_at, updated_at) "
            "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (rec["id"], rec.get("name", ""), rec.get("root_url", ""),
             rec.get("dataset", ""), rec.get("entity_type", ""),
             json.dumps(rec.get("field_map", {})), rec.get("mode", ""),
             json.dumps(rec.get("detail_urls", [])), json.dumps(rec.get("done_urls", [])),
             int(rec.get("row_count", 0)), rec.get("status", ""), rec.get("note", ""),
             rec.get("created_at") or now_iso(), now_iso()),
        )
        conn.commit()
    except Exception as e:
        log.debug("coll save: %s", e)


def _coll_load(cid: str) -> Optional[Dict]:
    _ensure_tables()
    try:
        row = _sqlite_conn().execute(
            "SELECT * FROM fabric_collections WHERE id=?", (cid,)).fetchone()
        if not row:
            return None
        d = dict(row)
        for k in ("field_map", "detail_urls", "done_urls"):
            try:
                d[k] = json.loads(d.get(k) or ("{}" if k == "field_map" else "[]"))
            except Exception:
                d[k] = {} if k == "field_map" else []
        return d
    except Exception as e:
        log.debug("coll load: %s", e)
        return None


async def _coll_link_provenance(entity_type: str, entry_id: str, name: str,
                                source_url: str, dataset: str):
    """Graph: entry node ──DERIVED_FROM──▶ source Page; entry ──IN_COLLECTION──▶ Dataset."""
    try:
        graph = _get_graph()
        if not (graph and graph.available):
            return
        await graph.upsert_node(entity_type, entry_id,
                                {"id": entry_id, "name": name[:120], "url": source_url})
        await graph.upsert_node("Page", source_url, {"id": source_url})
        await graph.link(entity_type, entry_id, "Page", source_url, rel="DERIVED_FROM")
        await graph.upsert_node("Dataset", dataset, {"id": dataset})
        await graph.link(entity_type, entry_id, "Dataset", dataset, rel="IN_COLLECTION")
    except Exception as e:
        log.debug("coll provenance %s: %s", entry_id, e)


@capability(
    "fabric.collection.detect",
    http_method="POST", http_path="/fabric/collection/detect", http_tags=["fabric"],
    memory="off",
    description="Detect a multi-page structured collection from a list/index URL. "
                "Samples linked detail pages, induces a consistent field map, and "
                "persists a 'detected' collection ready for reconstruct. "
                "Input: url (str! the index/list page), name (str), "
                "detail_pattern (regex to filter detail-page URLs), sample (int=5), "
                "field_map_mode (auto|heuristic|template_llm|per_page_llm). "
                "Output: {collection_id, name, fields, est_count, mode, sample_records}.",
)
async def fabric_collection_detect(
    url: str = "",
    name: str = "",
    detail_pattern: str = "",
    sample: int = 5,
    field_map_mode: str = "auto",
    trace_id=None,
) -> Dict:
    if not url or not url.strip():
        return {"error": "url is required"}
    if field_map_mode not in _COLL_MODES:
        return {"error": f"field_map_mode must be one of {_COLL_MODES}"}
    html, final_url = await _coll_fetch(url)
    if not html:
        return {"error": f"could not fetch {url}"}
    detail_urls = _extract_detail_links(html, final_url, detail_pattern)
    if not detail_urls:
        return {"error": "no detail-page links found — try a detail_pattern regex"}

    # Sample a handful of detail pages and extract their kv structure
    sample_urls = detail_urls[: max(1, min(int(sample or 5), 10))]
    samples: List[Dict[str, str]] = []
    for su in sample_urls:
        shtml, _ = await _coll_fetch(su)
        if shtml:
            kv = _extract_kv(shtml)
            if kv:
                samples.append(kv)
        await asyncio.sleep(0.2)

    # Field-map induction + effective-mode decision
    field_map: Dict[str, str] = {}
    key_counts: Dict[str, int] = {}
    for s in samples:
        for k in s:
            key_counts[k] = key_counts.get(k, 0) + 1
    common = [k for k, n in key_counts.items() if n >= max(1, len(samples) // 2)]
    regular = len(common) >= 3 and len(samples) >= 2

    eff_mode = field_map_mode
    if field_map_mode == "auto":
        # Regular layouts → cheap heuristic; irregular → per-page LLM (small sets)
        eff_mode = "heuristic" if (regular or not samples) else "per_page_llm"
    if eff_mode == "template_llm":
        # Canonicalise the field map ONCE; heuristic apply still works if this fails
        field_map = await _llm_field_map(samples)
    fields = sorted({k for s in samples for k in _apply_field_map(s, field_map).keys()})

    cid = "coll_" + hashlib.sha1(final_url.encode()).hexdigest()[:18]
    nm = name.strip() or (samples[0].get("title", "") if samples else "") or final_url
    rec = {
        "id": cid, "name": nm, "root_url": final_url,
        "dataset": "", "entity_type": _coll_key(nm) or "entry",
        "field_map": field_map, "mode": eff_mode,
        "detail_urls": detail_urls, "done_urls": [],
        "row_count": 0, "status": "detected", "note": "",
        "created_at": now_iso(),
    }
    _coll_save(rec)
    await emit_event({"type": "fabric.collection.detected", "collection_id": cid,
                      "name": nm, "detail_count": len(detail_urls),
                      "fields": fields, "mode": eff_mode})
    return {"collection_id": cid, "name": nm, "fields": fields,
            "est_count": len(detail_urls), "mode": eff_mode,
            "sample_records": samples[:3]}


@capability(
    "fabric.collection.reconstruct",
    http_method="POST", http_path="/fabric/collection/reconstruct", http_tags=["fabric"],
    memory="on", streams=["fabric.collection.progress"],
    description="Reconstruct a detected collection into a faithful internal dataset + "
                "graph. Crawls every detail page, extracts a uniform record per the "
                "field map, ingests them as one typed dataset, and links each entry "
                "DERIVED_FROM its source page (resumable — skips pages already done). "
                "Input: collection_id (str! from detect) OR url+name to detect inline; "
                "field_map_mode (override), limit (int=0=all), concurrency (int=4). "
                "Output: {collection_id, dataset, rows, done, total, status}.",
)
async def fabric_collection_reconstruct(
    collection_id: str = "",
    url: str = "",
    name: str = "",
    detail_pattern: str = "",
    ...
    rec = _coll_load(collection_id) if collection_id else None
    if not rec and url:
        det = await fabric_collection_detect(url=url, name=name,
                                             detail_pattern=detail_pattern,
                                             field_map_mode=field_map_mode or "auto")
        if det.get("error"):
            return det
        rec = _coll_load(det["collection_id"])
    if not rec:
        return {"error": "collection not found — pass collection_id or url"}

    mode = field_map_mode or rec.get("mode") or "heuristic"
    field_map = rec.get("field_map") or {}
    entity_type = rec.get("entity_type") or "entry"
    dataset = rec.get("dataset") or f"{_coll_key(rec.get('name','collection'))}.collection"
    detail_urls = rec.get("detail_urls") or []
    done = set(rec.get("done_urls") or [])
    todo = [u for u in detail_urls if u not in done]
    if limit and limit > 0:
        todo = todo[:limit]
    total = len(detail_urls)

    rec.update({"dataset": dataset, "mode": mode, "status": "running"})
    _coll_save(rec)

    sem = asyncio.Semaphore(max(1, min(int(concurrency or 4), 12)))
    new_records: List[Dict] = []
    lock = asyncio.Lock()

    async def _one(u: str):
        async with sem:
            html, final = await _coll_fetch(u)
            if not html:
                return
            kv = _extract_kv(html)
            if mode == "per_page_llm":
                fields = sorted({k for k in (field_map.values() if field_map else kv.keys()) if k})
                rec_data = await _llm_record_from_page(html, fields or list(kv.keys()))
                if not rec_data:
                    rec_data = _apply_field_map(kv, field_map)
            else:
                rec_data = _apply_field_map(kv, field_map)
            if not rec_data:
                return
            entry_name = (rec_data.get("title") or rec_data.get("name")
                          or kv.get("title") or u)
            entry_id = entity_type + "_" + hashlib.sha1(u.encode()).hexdigest()[:16]
            record = {
                "id": entry_id,
                "name": str(entry_name)[:200],
                **{k: v for k, v in rec_data.items() if k not in ("id",)},
                "_source_url": final,
                "_collection": rec["id"],
            }
            async with lock:
            ...
            await _coll_link_provenance(entity_type, entry_id, str(entry_name),
                                        final, dataset)
            await emit_event({"type": "fabric.collection.progress",
                              "collection_id": rec["id"], "name": str(entry_name)[:80],
                              "done": len(done), "total": total})

    # Process in batches so progress + done_urls persist incrementally (resumable)
    BATCH = 25
    for i in range(0, len(todo), BATCH):
        batch = todo[i:i + BATCH]
        await asyncio.gather(*[_one(u) for u in batch], return_exceptions=True)
        if new_records:
            try:
                await _ingest()(dataset, list(new_records), source="fabric_collection",
                                tags=["collection", entity_type, "data"])
            except Exception as e:
                log.warning("collection ingest %s: %s", dataset, e)
            new_records.clear()
        rec.update({"done_urls": sorted(done), "row_count": len(done),
                    "status": "running"})
        _coll_save(rec)

    rec.update({"done_urls": sorted(done), "row_count": len(done), "status": "done"})
    _coll_save(rec)
    await _link_graph(dataset, dataset, "IS_COLLECTION",
                      child_props={"name": rec.get("name", ""), "rows": len(done)})
    await emit_event({"type": "fabric.collection.progress", "collection_id": rec["id"],
                      "done": len(done), "total": total, "status": "done"})
    return {"collection_id": rec["id"], "dataset": dataset, "rows": len(done),
            "done": len(done), "total": total, "status": "done"}


@capability(
    "fabric.collection.list",
    http_method="GET", http_path="/fabric/collection/list", http_tags=["fabric"],
    memory="off", silent=True,
    description="List reconstructed/detected collections. Output: {collections:[...]}.",
)
async def fabric_collection_list(trace_id=None) -> Dict:
    _ensure_tables()
    try:
        rows = _sqlite_conn().execute(
            "SELECT id, name, root_url, dataset, mode, row_count, status, updated_at "
            "FROM fabric_collections ORDER BY updated_at DESC LIMIT 200").fetchall()
...
@capability(
    "fabric.collection.get",
    http_method="GET", http_path="/fabric/collection/get", http_tags=["fabric"],
    memory="off", silent=True,
    description="Fetch one collection's metadata + a few sample records. "
                "Input: collection_id (str!). Output: {collection, sample_records}.",
)
async def fabric_collection_get(collection_id: str = "", trace_id=None) -> Dict:
    rec = _coll_load(collection_id)
    if not rec:
    ...
    if ds:
        try:
            rows = _sqlite_conn().execute(
                "SELECT text, data FROM fabric_records WHERE dataset_id=? LIMIT 5",
                (ds,)).fetchall()
            for r in rows:
                try:
                    samples.append(json.loads(r["data"]) if r["data"] else {"text": r["text"]})
                except Exception:
                    samples.append({"text": r["text"]})
        except Exception as e:
            log.debug("coll get samples: %s", e)
    # Trim large url lists for the response
    out = {k: v for k, v in rec.items() if k not in ("detail_urls", "done_urls")}
    out["detail_count"] = len(rec.get("detail_urls") or [])
    out["done_count"] = len(rec.get("done_urls") or [])
    return {"collection": out, "sample_records": samples}
...
@capability(
    "fabric.entity_graph.extract_text",
    http_method="POST", http_path="/fabric/entity_graph/extract_text",
    http_tags=["fabric", "graph", "entity"], memory="off",
    description=(
        "Run entity + relationship extraction over caller-supplied text items "
        "instead of a stored dataset. Lets the Loom side-panel parse the nodes "
        "of the *active* graph (their labels / titles / text) through the same "
        "NER engine used by the dataset pipeline, without first ingesting them. "
        "Nothing is persisted. "
        "Input: items (list of {id, text}), content_type (str='text'). "
        "Output: {ok, nodes:[{id,name,type,props:{mention_count,record_ids}}], "
        "edges:[{from,to,rel,props:{score,cue}}], entities, relations}."
    ),
)
async def cap_entity_graph_extract_text(
    ...
    items = items or []
    if not items:
        return {"error": "items required (list of {id, text})"}

    # The relationship inference engine is shared with web acquisition; entity
    # extraction goes through extract_entities2 (the same canonical engine).
    _extract_ents, extract_rels, _persist = _wa_entity_fns()

    def _eid(etype: str, norm: str) -> str:
        return f"{etype}:{hashlib.sha1((norm or '').encode()).hexdigest()[:12]}"

    ents: Dict[str, Dict] = {}          # eid -> node
    edges_best: Dict[Tuple[str, str], Dict] = {}
    ...
    def _process() -> None:
        for it in items:
            ...
            if not text or not text.strip():
                continue
            ent_list = extract_entities2(text, content_type) or []
            for e in ent_list:
                norm = e.get("normalised") or (e.get("name") or "").lower()
                if not norm:
                    continue
                etype = e.get("type", "named_entity")
                eid = _eid(etype, norm)
                node = ents.get(eid)
                if node is None:
                    node = ents[eid] = {
                        "id": eid, "name": e.get("name") or norm, "type": etype,
                        "props": {"mention_count": 0, "record_ids": []},
                    }
                node["props"]["mention_count"] += int(e.get("mention_count", 1) or 1)
                if rid and rid not in node["props"]["record_ids"]:
                    node["props"]["record_ids"].append(rid)

            if not extract_rels:
                continue
            try:
                rels = extract_rels(ent_list, text) or []
            except Exception:
                rels = []
            for r in rels:
                fnorm = (r.get("from_name") or "").lower()
                tnorm = (r.get("to_name") or "").lower()
                if not fnorm or not tnorm:
                    continue
                fid = _eid(r.get("from_type", "named_entity"), fnorm)
                tid = _eid(r.get("to_type", "named_entity"), tnorm)
                if fid == tid:
                    continue
                key = (fid, tid) if fid < tid else (tid, fid)
                score = r.get("score", 0) or 0
                prev = edges_best.get(key)
                if prev and prev["props"].get("score", 0) >= score:
                    continue
                edges_best[key] = {
                    "from": fid, "to": tid, "rel": r.get("rel", "RELATED_TO"),
                    "props": {"score": score, "cue": r.get("cue", "")},
                }

    loop = asyncio.get_running_loop()
    await loop.run_in_executor(None, _process)

    nodes = list(ents.values())
    edges = list(edges_best.values())
    return {
        "ok": True,
        "nodes": nodes,
        "edges": edges,
...
"""
    page = (
        ...
        # vera-ui.js makes this standalone page track the LIVE theme like every
        # other panel: it loads /ui/themes.css and applies the active theme's
        # CSS vars (incl. the --bg0/--acc orchestrator namespace the graph uses)
        # as inline styles on <html>, overriding the static _CSS_VARS fallback
        # above. Loaded before the graph scripts so the vars are in place when
        # the graph initialises. Without it the Discover graph stayed on the
        # hardcoded palette while the rest of the fabric panel re-themed.
        ...
        "<script>"
        # This standalone page IS Discover's dedicated home - let the Discover
        # panel auto-restore the active crawl + default-open here (the sidebar
        # tab on other graphs stays inert until the user acts).
        "window.VERA_AUTOLOAD_discover=true;"
