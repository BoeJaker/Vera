...
class MemoryGraphAdapter(GraphAdapter):
    ...
    def __init__(self):
        self._mem = None
        try:
            from Vera.vera import memory as _m
...
async def fabric_graphs_snapshot(graph: str = "fabric", limit: int = 200,
    ...
    try:
        ...
        limit = 200
    # Ceiling raised from 2000 → 20000 so populated instances can render their
    # full structural graph in one snapshot rather than a non-deterministic
    # truncated subset. Visualisation performance is handled client-side by the
    # spatial-grid physics in vera_graph.js.
    ...
    if dataset_id:
        ...
        node_q = ("MATCH (n) WHERE (n:Dataset AND n.id=$dsid) "
                  ...
                  "RETURN n ORDER BY id(n) LIMIT $limit")
        edge_q = ("MATCH (a:FabricRecord)-[r]-(b:FabricRecord) "
                  "WHERE a.dataset_id=$dsid AND b.dataset_id=$dsid "
                  "AND type(r) <> 'CONTAINS' "
                  "RETURN a, r, b ORDER BY id(r) LIMIT $limit")
        params = {"dsid": dataset_id, "limit": limit}
    elif labels:
        labels_safe = [re.sub(r"[^A-Za-z0-9_]", "", l) for l in labels if l]
        # ORDER BY id(n) makes truncation deterministic — without it Neo4j may
        # return a different LIMIT-sized subset each call, which made the
        # structure graph flip between node counts on repeated loads.
        node_q = "MATCH (n) WHERE any(l IN labels(n) WHERE l IN $labels) RETURN n ORDER BY id(n) LIMIT $limit"
        edge_q = ("MATCH (a)-[r]->(b) "
        ...
        node_q = "MATCH (n) RETURN n ORDER BY id(n) LIMIT $limit"
        edge_q = "MATCH (a)-[r]->(b) RETURN a, r, b ORDER BY id(r) LIMIT $limit"
        params = {"limit": limit}
...
SOURCE_TYPES: Dict[str, Dict] = {
    ...
    "elasticsearch": {"description": "Elasticsearch / OpenSearch",
                       ...
                       "needs_url": False},
    "index":    {"description": "Index of sources — a CSV/JSON/HTML list of other "
                                "resources (e.g. a .gov domain CSV, an IP-cam directory). "
                                "Ingests the list as a dataset and can fan out to child "
                                "sources, authority priors, and discovery seeds.",
                 "config_fields": [
                     {"name":"url_col","label":"URL/domain column",
                      "help":"CSV header or JSON key holding the resource URL or domain "
                             "(blank = first column / auto-detect)"},
                     {"name":"label_col","label":"Label column",
                      "help":"optional column for a human label"},
                     {"name":"tags_col","label":"Tags column",
                      "help":"optional column whose value becomes a tag"},
                     {"name":"url_prefix","label":"URL prefix",
                      "help":"prepended to bare values, e.g. https:// for a domain list"},
                     {"name":"filter","label":"Row filter (regex)",
                      "help":"keep a row only if its url/label matches this regex"},
                     {"name":"register_children","label":"Register child sources",
                      "type":"select","options":["false","true"],"default":"false",
                      "help":"create a child source per row (default just ingests the list)"},
                     {"name":"child_source_type","label":"Child source type",
                      "type":"select","options":["scrape","rss","api","http","recon"],
                      "default":"scrape"},
                     {"name":"child_interval","label":"Child interval (s)","type":"number",
                      "default":"0","help":"0 = manual; children inherit no auto-pull"},
                     {"name":"child_tags","label":"Child tags (comma-sep)"},
                     {"name":"seed_discovery","label":"Seed discovery crawl",
                      "type":"select","options":["false","true"],"default":"false"},
                     {"name":"mark_authority","label":"Mark domains authoritative",
                      "type":"select","options":["false","true"],"default":"false"},
                     {"name":"topic_key","label":"Authority topic key",
                      "help":"topic_key used when marking domain authority"},
                     {"name":"base_authority","label":"Base authority","type":"number",
                      "default":"5","help":"prior weight (1-10) for marked domains"},
                     {"name":"limit","label":"Max rows","type":"number","default":"5000"}],
                 "auth_fields":   [{"name":"headers","label":"Headers (JSON)",
                                    "type":"textarea"}],
                 "needs_url": True},
}
...
def _index_slug(s: str) -> str:
    """Deterministic short slug for a child-source id derived from a URL/value."""
    s = (s or "").strip().lower()
    s = re.sub(r"^https?://", "", s)
...
def _index_config(src: dict) -> dict:
    """Parse the JSON config blob off an index source (tolerant of dict or str)."""
    cfg = src.get("config")
    if isinstance(cfg, dict):
        return cfg
    if isinstance(cfg, str) and cfg.strip():
...
def _index_parse_rows(payload: str, content_type: str, final_url: str) -> List[dict]:
    """Turn an index payload into a list of {column: value} row dicts.

    Handles CSV/TSV (header row), a JSON array of objects/strings, or an HTML
    page of links (each anchor → {"url","label"}). Returns [] if nothing usable.
    """
    import csv, io
    from urllib.parse import urljoin

    txt = payload.lstrip("﻿").strip()
    ...
    if txt[:1] in ("[", "{") or "json" in ct:
        try:
            obj = json.loads(txt)
            arr = obj if isinstance(obj, list) else (obj.get("data") or obj.get("items") or [])
            rows = []
            for el in arr:
    ...
    # CSV / TSV — sniff the delimiter from the first line
    looks_csv = ("csv" in ct or "tab-separated" in ct
                 or ("," in txt.split("\n", 1)[0]) or ("\t" in txt.split("\n", 1)[0]))
    if looks_csv:
        try:
            sample = txt[:4096]
...
def _index_pick(row: dict, col: str) -> str:
    """Case-insensitive column lookup with graceful fallback."""
    if not row:
        return ""
    if col:
        if col in row:
        ...
        if col.lower() in low:
            return str(row[low[col.lower()]] or "").strip()
        return ""
    # No column given — take the first non-empty value
    for v in row.values():
...
async def _mark_domain_authority(domain: str, topic_key: str, base_authority: float,
                                 source_type: str = "index") -> None:
    """Record a curated domain as an authority prior in fabric_domain_authority
    (created lazily by the discovery module — same SQLite DB)."""
    disc = sys.modules.get("discovery") or sys.modules.get("Vera.vera.fabric.discovery")
    try:
        if disc and hasattr(disc, "_ensure_tables"):
            disc._ensure_tables()
        await _enqueue_write({
            "kind": "raw",
            "sql": ("INSERT INTO fabric_domain_authority "
                    "(domain, topic_key, source_type, base_authority, score, updated_at) "
                    "VALUES (?,?,?,?,?,?) "
                    "ON CONFLICT(domain, topic_key) DO UPDATE SET "
                    "base_authority=excluded.base_authority, "
                    "source_type=excluded.source_type, updated_at=excluded.updated_at"),
            "params": (domain, topic_key or "", source_type,
                       float(base_authority), float(base_authority), now_iso()),
        }, wait=False)
    except Exception as e:
        log.debug("mark_domain_authority %s: %s", domain, e)


async def _pull_index(src: dict) -> List[dict]:
    """Index / source-of-sources pull.

    The source URL points at a *list* of other resources (a CSV of domains, a
    JSON array, or an HTML page of links). We:
      1. Ingest the list itself as a dataset — one record per row — so the
         catalogue is queryable / graphable internally (the normal ingest path
         handles the returned items).
      2. Optionally fan out (config flags): register a child source per row,
         mark each domain authoritative, and/or seed a discovery crawl.

    Child sources use a deterministic id ("<index_id>:<slug>") so re-pulls are
    idempotent; rows that vanish on a later pull disable their stale children.

    config keys: url_col, label_col, tags_col, url_prefix, filter,
      register_children, child_source_type, child_interval, child_tags,
      seed_discovery, mark_authority, topic_key, base_authority, limit.
    """
    ...
    cfg   = _index_config(src)
    rows  = _index_parse_rows(payload, ctype, final_url)
    limit = int(cfg.get("limit", 5000) or 5000)
    if not rows:
        await emit_event({"type": "fabric.source.index.expanded", "label": src.get("label","?"),
                          "registered": 0, "disabled": 0, "rows": 0,
                          "note": "no rows parsed from index payload"})
        return []

    def _truthy(v) -> bool:
        return str(v).strip().lower() in ("1", "true", "yes", "on")

    url_col      = cfg.get("url_col", "")
    label_col    = cfg.get("label_col", "")
    tags_col     = cfg.get("tags_col", "")
    url_prefix   = cfg.get("url_prefix", "")
    filt_re      = None
    if cfg.get("filter"):
        try:
            filt_re = re.compile(cfg["filter"], re.I)
        except Exception:
            filt_re = None
    register_children = _truthy(cfg.get("register_children", False))
    seed_discovery    = _truthy(cfg.get("seed_discovery", False))
    mark_authority    = _truthy(cfg.get("mark_authority", False))
    child_type        = cfg.get("child_source_type", "scrape") or "scrape"
    child_interval    = int(cfg.get("child_interval", 0) or 0)
    base_authority    = float(cfg.get("base_authority", 5) or 5)
    topic_key         = cfg.get("topic_key", "")
    child_tags        = [t.strip() for t in str(cfg.get("child_tags", "")).split(",") if t.strip()]

    items: List[dict] = []
    new_child_ids: set = set()
    registered = 0
    seeds: List[str] = []

    for row in rows[:limit]:
        raw = _index_pick(row, url_col)
        if not raw:
            continue
        if url_prefix and not raw.startswith(("http://", "https://")):
            child_url = (url_prefix + raw if url_prefix.endswith(("://", "/"))
                         else url_prefix + "/" + raw)
        else:
            child_url = raw
        label = _index_pick(row, label_col) or raw
        if filt_re and not (filt_re.search(child_url) or filt_re.search(label)):
            continue
        tags = list(child_tags)
        if tags_col:
            tv = _index_pick(row, tags_col)
            if tv:
                tags.append(tv)

        # 1. record for the catalogue dataset (all original columns preserved)
        items.append({
            "text": (label + " — " + child_url)[:2000],
            "data": {**{str(k): row[k] for k in row}, "url": child_url, "label": label,
                     "index_source": src.get("id", "")},
        })

        domain = ""
        try:
            domain = urlparse(child_url if child_url.startswith(("http://", "https://"))
                              else "https://" + child_url).hostname or ""
        except Exception:
            domain = ""

        if mark_authority and domain:
            await _mark_domain_authority(domain, topic_key, base_authority)

        if register_children:
            cid = f"{src['id']}:{_index_slug(child_url)}"
            new_child_ids.add(cid)
            try:
                await fabric_sources_add(
                    url=child_url, source_type=child_type, label=label[:60],
                    interval=child_interval, tags=",".join(tags),
                    enabled=True, id=cid,
                )
                registered += 1
            except Exception as e:
                log.debug("index child add %s: %s", cid, e)

        if seed_discovery and child_url:
            seeds.append(child_url)

    # Diff: disable child sources we previously created that are no longer listed
    disabled = 0
    if register_children:
        prefix = f"{src['id']}:"
        for cid, csrc in list(_SOURCES.items()):
            if cid.startswith(prefix) and cid not in new_child_ids and csrc.get("enabled", True):
                try:
                    csrc["enabled"] = False
                    _SOURCES[cid] = csrc
                    await _sqlite_upsert_source(csrc)
                    disabled += 1
                except Exception as e:
                    log.debug("index child disable %s: %s", cid, e)

    # Optionally seed a discovery crawl over the listed URLs (fire-and-forget)
    if seed_discovery and seeds:
        try:
            from Vera.vera.capability_orchestration import CAPABILITY_REGISTRY
        except ImportError:
            CAPABILITY_REGISTRY = {}
        crawl_fn = (CAPABILITY_REGISTRY.get("fabric.discover.crawl") or {}).get("fn")
        if crawl_fn:
            async def _seed():
                for u in seeds[:200]:
                    try:
                        await crawl_fn(url=u)
                    except Exception as e:
                        log.debug("index seed crawl %s: %s", u, e)
            asyncio.create_task(_seed())

    await emit_event({"type": "fabric.source.index.expanded",
                      "label": src.get("label", "?"),
                      "rows": len(items), "registered": registered,
                      "disabled": disabled, "seeded": len(seeds) if seed_discovery else 0,
                      "authority": mark_authority})
    return items
...
@capability(
    ...
    description="Search the data fabric across all stored datasets using keyword and/or semantic (vector) search. "
                "WHEN TO USE: when you need to look up records, documents, or data from structured datasets; "
                "when the user asks to 'search the fabric', 'query data', or 'find records'. "
                "HOW TO USE: call fabric.datasets first to see what datasets exist. "
                "Then query with text= for keyword search, vector= for semantic similarity, or both. "
...
async def cap_fabric_datasets(parent: str = "", trace_id=None) -> Dict:
    ...
    if parent:
        # Return the immediate next level under `parent.`, collapsing everything
        # deeper into namespace groups. A next-level segment may be:
        #   • a real dataset (e.g. parent='cve' → 'cve.nvd' exists), or
        #   • a synthetic namespace that only has descendants (e.g.
        #     parent='this' → 'this.is' when only 'this.is.an.example' exists).
        # Synthesising intermediate namespaces is what lets the structure graph
        # expand arbitrarily deep dotted names one segment at a time.
        ...
        for d in all_ds:
            ...
            seg = rest.split(".", 1)[0]
            child_id = prefix + seg
            g = groups.get(child_id)
            if g is None:
                g = {"dataset_id": child_id, "record_count": 0,
                     "has_children": False, "synthetic": child_id not in all_ids}
                groups[child_id] = g
            if did == child_id:
                # This row IS the immediate child dataset — carry its real fields.
                g["synthetic"] = False
                g["record_count"] = d.get("record_count", 0) or g["record_count"]
                for k in ("tags", "source_id", "created_at", "updated_at", "description"):
                    if k in d and d[k] not in (None, ""):
                        g[k] = d[k]
            else:
                # Deeper descendant — this child has its own children, and we
...
@capability(
    ...
    description="Diagnostic statistics from all fabric storage backends. "
                "USE FOR: checking which storage backends are active, total record counts, storage sizes. "
                "Output: {postgres, faiss, chroma, neo4j, sqlite, object_store} — each with available (bool), record_count, and backend-specific stats.",
)
...
@capability(
    ...
    memory="off",
    description="Register a data source (RSS, API, wiki, HTTP, scrape, recon, index). "
                "Input: url (str!), source_type (rss|api|http|wiki|scrape|recon|index), "
                "label (str), dataset_id (str), interval (int seconds, 0=manual), "
                "tags (comma-sep), headers (JSON), jq_path (str), "
                "page (str, wiki only), limit (int), config (JSON). "
                "For source_type=index, config selects the list columns and fan-out: "
                "{url_col, label_col, tags_col, url_prefix, filter, register_children, "
                "child_source_type, child_interval, child_tags, seed_discovery, "
                "mark_authority, topic_key, base_authority, limit}. "
...
@capability(
    "fabric.sources.add_index",
    http_method="POST", http_path="/fabric/sources/add_index", http_tags=["fabric"],
    memory="off",
    description="Register an INDEX source — a URL pointing at a list of other resources "
                "(a CSV of domains, a JSON array, or an HTML page of links). On pull it "
                "ingests the list as a dataset and can fan out to child sources, authority "
                "priors, and discovery seeds. "
                "Input: url (str!), label (str), url_col (str, the column/key holding each "
                "resource URL or domain), interval (int seconds; weekly=604800, daily=86400, "
                "default 86400), label_col (str), tags_col (str), url_prefix (str, e.g. "
                "'https://' for a bare-domain list), filter (regex), register_children (bool), "
                "child_source_type (str default scrape), child_interval (int default 0), "
                "child_tags (comma-sep), seed_discovery (bool), mark_authority (bool), "
                "topic_key (str), base_authority (float default 5), limit (int default 5000), "
                "tags (comma-sep), headers (JSON). Output: {id, label, dataset_id}.",
)
async def fabric_sources_add_index(
    url:               str   = "",
    label:             str   = "",
    url_col:           str   = "",
    interval:          int   = 86400,
    ...
    url_prefix:        str   = "",
    filter:            str   = "",
    register_children: bool  = False,
    child_source_type: str   = "scrape",
    child_interval:    int   = 0,
    child_tags:        str   = "",
    seed_discovery:    bool  = False,
    mark_authority:    bool  = False,
    topic_key:         str   = "",
    base_authority:    float = 5.0,
    limit:             int   = 5000,
    tags:              str   = "",
    headers:           str   = "{}",
    trace_id=None,
    ...
    cfg = {
        "url_col": url_col, "label_col": label_col, "tags_col": tags_col,
        "url_prefix": url_prefix, "filter": filter,
        "register_children": bool(register_children),
        "child_source_type": child_source_type, "child_interval": int(child_interval or 0),
        "child_tags": child_tags, "seed_discovery": bool(seed_discovery),
        "mark_authority": bool(mark_authority), "topic_key": topic_key,
        "base_authority": float(base_authority or 5), "limit": int(limit or 5000),
    }
    return await fabric_sources_add(
        url=url, source_type="index", label=label, interval=int(interval or 0),
        tags=tags, headers=headers, config=json.dumps(cfg),
    )
...
async def fabric_skills_build(
    ...
    try:
        from Vera.vera.skills import skills_create, ontologies_create
...
async def fabric_extract_graph(
    ...
    # Identifies code symbols, URLs, proper nouns, quoted terms.
    # Relationships are typed by sentence-level proximity:
    #   same sentence → CO_OCCURS (strong); same record only → MENTIONED_WITH (weak).
    if mode in ("nlp", "hybrid"):
        pat_camel    = re.compile(r"\b([A-Z][a-z]+(?:[A-Z][a-z]+){1,})\b")
        pat_snake    = re.compile(r"\b([a-z]+(?:_[a-z]+){2,})\b")  # ≥2 underscores avoids noise
        ...
        # Simple sentence splitter — split on ". ", "! ", "? ", newlines
        pat_sent     = re.compile(r'(?<=[.!?\n])\s+')

        # co-occurrence weight accumulator: (from_id, to_id, rel) → count
        _rel_weight: Dict[tuple, int] = {}

        def _add_rel(a_id, b_id, rel, w=1):
            # Normalise direction so A<B always (avoids duplicate forward/reverse edges)
            key = (min(a_id, b_id), max(a_id, b_id), rel)
            _rel_weight[key] = _rel_weight.get(key, 0) + w
        ...
        for rec in records:
            ...
            for m in pat_func.finditer(text):
                ...
                if name: local_ents.add(("function", name))
            for m in pat_import.finditer(text):
                local_ents.add(("module", m.group(1)))
            for m in pat_camel.finditer(text):
                local_ents.add(("concept", m.group(1)))
            for m in pat_snake.finditer(text):
                local_ents.add(("identifier", m.group(1)))
            ...
            # Register entities and build id lookup
            local_ids: list = []
            ...
            # Sentence-level proximity — entities in the same sentence get
            # CO_OCCURS (weight 3); all document pairs get MENTIONED_WITH (weight 1).
            sentences = pat_sent.split(text)
            for sent in sentences:
                sent_ids = [
                    eid for eid in local_ids
                    if entities[eid]["name"].lower() in sent.lower()
                ]
                for i in range(len(sent_ids)):
                    for j in range(i + 1, len(sent_ids)):
                        _add_rel(sent_ids[i], sent_ids[j], "CO_OCCURS", w=3)

            # Weaker document-level co-occurrence for all remaining pairs
            for i in range(len(local_ids)):
                for j in range(i + 1, len(local_ids)):
                    _add_rel(local_ids[i], local_ids[j], "MENTIONED_WITH", w=1)

        # Convert weight accumulator to relation list; keep top relations by weight
        for (a_id, b_id, rel), weight in sorted(
                _rel_weight.items(), key=lambda x: -x[1])[:5000]:
            relations.append({
                "from_id": a_id, "to_id": b_id,
                "rel": rel,
                "weight": weight,
                "from_name": entities.get(a_id, {}).get("name", a_id),
                "to_name":   entities.get(b_id, {}).get("name", b_id),
            })
    ...
    if mode in ("llm", "hybrid"):
        ...
        if ollama_generate:
            ...
            batch_size = 8
            llm_record_cap = min(len(records), 200)
            for i in range(0, llm_record_cap, batch_size):
                chunk = records[i:i + batch_size]
                texts = "\n\n".join(
                    f"[id={r['id']}] {(r.get('text') or '')[:600]}"
                    for r in chunk
                )[:8000]
                prompt = (
                    "Extract named entities and typed relationships from these records.\n"
                    "Return ONLY a JSON object with this exact structure:\n"
                    '{"entities":[{"name":"...","type":"person|organization|technology|'
                    'concept|tool|product|place|event|method|dataset"}],'
                    '"relations":[{"from":"entity name","to":"entity name",'
                    '"rel":"USES|PART_OF|RELATES_TO|CREATED_BY|DEPENDS_ON|'
                    'IMPLEMENTS|EXTENDS|REFERENCES|LOCATED_IN|PARTICIPATES_IN|'
                    'CAUSES|PRECEDES|SIMILAR_TO|CONTRASTS_WITH"}]}\n'
                    "Extract up to 30 entities and 50 typed relationships. "
                    "Prefer specific relationship types over RELATES_TO.\n\n"
                    f"Records:\n{texts}"
                )
                try:
                    raw = await asyncio.wait_for(
                        ollama_generate(prompt, system="You output strict JSON only.", json_mode=True),
                        timeout=90,
                    )
                    raw = raw.strip()
                    if raw.startswith("```"):
                        raw = re.sub(r"^```(?:json)?\n?|\n?```$", "", raw)
                    parsed = json.loads(raw)
                    for e in (parsed.get("entities") or [])[:30]:
                        if not isinstance(e, dict): continue
                        name = (e.get("name") or "").strip()
                        if not name or len(name) < 2: continue
                        etype = (e.get("type") or "concept")[:30]
                        eid = etype + ":" + hashlib.sha1(name.encode()).hexdigest()[:12]
                        ent = entities.setdefault(eid, {"id": eid, "name": name,
                                                         "type": etype, "mentions": []})
                        for r in chunk:
                    ...
                    for r in (parsed.get("relations") or [])[:50]:
                        if not isinstance(r, dict): continue
                        f_id = name_to_id.get(r.get("from"))
                        t_id = name_to_id.get(r.get("to"))
                        if f_id and t_id and f_id != t_id:
                            rel_label = (r.get("rel") or "RELATES_TO")[:40].upper()
                            relations.append({
                                "from_id": f_id, "to_id": t_id,
                                "rel": rel_label,
                                "weight": 2,
                                "from_name": entities[f_id]["name"],
                                "to_name":   entities[t_id]["name"],
                            })
                except asyncio.TimeoutError:
    ...
    # Deduplicate relations — keep highest weight when same pair+rel appears multiple times
    seen_rels: Dict[tuple, int] = {}   # key → index in deduped
    deduped: list = []
    for r in relations:
        key = (r["from_id"], r["to_id"], r["rel"])
        if key in seen_rels:
            existing = deduped[seen_rels[key]]
            existing["weight"] = existing.get("weight", 1) + r.get("weight", 1)
        else:
            seen_rels[key] = len(deduped)
            deduped.append(dict(r))
    # Sort by weight descending so the strongest relationships surface first
    deduped.sort(key=lambda x: -x.get("weight", 1))
    relations = deduped
    ...
    if persist and (entities or relations):
        ...
        # Persist entity-entity relationships to SQLite so the query endpoint
        # can return them without Neo4j. Neo4j also receives them above via
        # adapter.link_many — this is the SQLite mirror that serves as the
        # reliable fallback for cap_entity_graph_query.
        if relations:
            try:
                loop2 = asyncio.get_running_loop()
                _rels_snapshot = relations[:2000]
                def _write_relations():
                    conn2 = _sqlite_conn()
                    ts2 = now_iso()
                    for r in _rels_snapshot:
                        props = json.dumps({
                        ...
                        conn2.execute(
                            ...
                            "VALUES (?,?,?,?,?,?)",
                            (r["from_id"], r["to_id"], r["rel"],
                             props, dataset_id, ts2)
                        )
                    conn2.commit()
                    conn2.close()
                await loop2.run_in_executor(None, _write_relations)
            except Exception as _e2:
                log.debug("entity relations write: %s", _e2)
...
async def fabric_ontologies_build(
    ...
    ontology_id = None
    try:
        from Vera.vera.skills import ontologies_create
        ont_rec = await ontologies_create(
            name              = name.strip(),
            description       = final_desc,
...
from Vera.vera.capability_orchestration import register_ui as _reg_ui
