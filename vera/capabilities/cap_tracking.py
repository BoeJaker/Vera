...
_FACTORY_DEFAULTS: Dict[str, Any] = {
    ...
    "groups": {
        ...
        # Dream-layer persistence: when on, opt-in dream cycles/thoughts write
        # their graph (cycle/theme/finding nodes) into the memory graph's dream
        # layer. This group is the master switch for that flow.
        "dream":    {"track": True,  "budget": 200, "note": "dream layer → memory graph"},
...
def effective_tracked_groups() -> set:
    """
    Group-level view of what's currently tracked, ignoring per-cap overrides
    and the chain/rate flood guards. Used by consumers (e.g. the dream system)
    that want "what counts as real activity?" at group granularity.

    A group is tracked when:
      • it is not a hard-skip group, AND
      • its group config says track=True, OR it has no group config and
        default_track is True.

    Candidate groups are the union of explicitly-configured groups and the
    groups present in the live capability registry (so default_track=True is
    reflected for real groups, not just the ones named in config).
    """
    cfg = _CAP_TRACKING
    if not cfg.get("enabled", True):
        return set()

    candidates = set(cfg.get("groups", {}).keys())
    orch = _INSTALLED_ORCH
    if orch and hasattr(orch, "CAPABILITY_REGISTRY"):
        for cname in orch.CAPABILITY_REGISTRY:
            candidates.add(cname.split(".")[0])

    default_track = cfg.get("default_track", True)
    out = set()
    for group in candidates:
        if group in _HARD_SKIP_GROUPS:
            continue
        group_cfg = cfg.get("groups", {}).get(group)
        if group_cfg is not None:
            if group_cfg.get("track", False):
                out.add(group)
        elif default_track:
            out.add(group)
    return out
...
def tracked_group_prefixes() -> list:
    """Effective tracked groups as dotted prefixes, e.g. ['llm.', 'research.']."""
    return sorted(g + "." for g in effective_tracked_groups())
...
if _CAP_AVAILABLE:
    ...
    async def cap_tracking_set_session(session_id: str, trace_id=None):
        import Vera.vera.capability_orchestration as _co
        if session_id:
