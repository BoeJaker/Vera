...
async def _ollama_with_options(
    ...
    body: Dict[str, Any] = {
        "model":      use_model,
        "prompt":     prompt,
        # Stream the generation. With streaming the httpx read-timeout applies to
        # the gap BETWEEN tokens, not the whole generation — so a long review no
        # longer trips a fixed total-time wall the way stream=False did.
        "stream":     True,
        # Keep the model resident so a multi-file review run doesn't reload it
        # (and time out) on every file.
    ...
    # `timeout` is the per-read (inter-token) budget, not a total cap: streaming
    # resets it on every chunk, so a long generation only fails if the node
    # genuinely STALLS for `timeout` seconds between tokens. Connect/write stay
    # short so a dead node is detected quickly.
    _timeout = _httpx_inspect.Timeout(timeout, connect=20.0, write=30.0, pool=20.0)
    try:
        buf: List[str] = []
        _ntok = 0
        async with _httpx_inspect.AsyncClient(timeout=_timeout) as c:
            async with c.stream("POST", f"{url}/api/generate", json=body) as r:
                if r.status_code != 200:
                    ...
                    async for _chunk in r.aiter_bytes():
                        err_body += _chunk.decode("utf-8", errors="replace")
                    raise Exception(f"ollama {r.status_code}: {err_body[:300]}")
            ...
            log.info("ollama_done [%s] %.2fs tokens=%d caller=ide_inspect:%s",
                     _req_id, _elapsed, _ntok, _caller_func)
            _ollama_log_append({
                "req_id": _req_id, "model": use_model, "instance": chosen,
                "caller_file": "ide_inspect_capabilities.py", "caller_func": _caller_func,
                "prompt_preview": _prompt_preview, "ts": now_iso(),
                "status": "done", "elapsed_s": _elapsed, "tokens": _ntok,
            })
            try:
                await emit_event({
                    ...
                    "caller_file": "ide_inspect_capabilities.py", "caller_func": _caller_func,
                    "elapsed_s": _elapsed, "token_count": _ntok,
                })
            except Exception:
                pass
            return result
        ...
        _elapsed = round(_time.time() - _t0, 2)
        # httpx timeout exceptions stringify to "" — fall back to the type name
        # so the Ollama panel shows "ReadTimeout" instead of a bare "unknown".
        _errmsg = (str(e)[:200] or f"{type(e).__name__} (no message — likely timeout after {_elapsed}s)")
        log.warning("inspect _ollama_with_options [%s] FAILED after %.2fs: %s", chosen, _elapsed, _errmsg)
        _ollama_log_append({
            "req_id": _req_id, "model": use_model, "instance": chosen,
            "caller_file": "ide_inspect_capabilities.py", "caller_func": _caller_func,
            "prompt_preview": _prompt_preview, "ts": now_iso(),
            "status": "error", "elapsed_s": _elapsed, "error": _errmsg,
        })
        try:
            await emit_event({
                ...
                "model": use_model, "instance_id": chosen,
                "caller_file": "ide_inspect_capabilities.py", "caller_func": _caller_func,
                "elapsed_s": _elapsed, "error": _errmsg,
            })
        except Exception:
            pass
...
def _resolve_source_root() -> Path:
    ...
    try:
        from Vera.vera.ide import ide_capabilities as _ic
...
async def ide_inspect_review_file(
    ...
    try:
        ...
        model = OLLAMA_MODEL
        # Reviews are heavy (large context); always prefer the GPU node and route
        # model-aware so we land on a node that actually has the model rather than
        # the least-busy CPU node (which times out on 9B/large-ctx generations).
        iid = pick_instance(prefer_gpu=True, model=model)
        inst = OLLAMA_INSTANCES.get(iid or "", {})
        full = (_REVIEW_SYSTEM + "\n\n" + user_prompt)

        # Stream tokens so the review is observably alive (and never silently
        # blocks for the whole generation). Re-emit throttled progress on the
        # event bus so panels can show a live token count / tail.
        _acc: List[str] = []
        _seen = {"n": 0}

        async def _on_tok(tok: str):
            _acc.append(tok)
            _seen["n"] += 1
            if _seen["n"] % 12 == 0:
                try:
                    await emit_event({
                        "type": "ide.inspect.review.token",
        ...
        raw = await _ollama_with_options(
            ...
            prefer_gpu=True,
            options={"temperature": 0.15, "num_ctx": cfg.OLLAMA_NUM_CTX},
            stream_cb=_on_tok,
        )
        raw = (raw or "").strip()
...
async def ide_inspect_plan_improvement(
    ...
    try:
        ...
        system = (
            ...
            "Every @capability lives under Vera.vera/*.py. UI panels are "
...
async def ide_inspect_scaffold_capability(
    ...
    try:
        ...
        if "from Vera.vera" not in code and "import" not in code.split("\n", 5)[0]:
...
async def ide_inspect_scaffold_ui_panel(
    ...
    try:
        ...
        module_src = (
            ...
            "from pathlib import Path\n"
            "from fastapi.responses import HTMLResponse\n"
            "from Vera.vera.capability_orchestration import capability, register_ui\n\n"
...
try:
    from Vera.vera.capability_orchestration import schedule as _schedule
