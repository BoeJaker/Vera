...
def _today_preamble() -> str:
    """A one-line current-date prefix prepended to every research system prompt.

    The research models have no inherent knowledge of the current date and were
    defaulting to their training-era year when composing web-search queries and
    reports. Prepending this keeps all time-sensitive reasoning anchored to now.
    """
    from datetime import datetime as _dt
    d = _dt.now().astimezone()
    return (f"Current date: {d:%A, %B %d, %Y} (year {d:%Y}). "
            "Use this date for all time-sensitive reasoning and when composing "
...
async def debug_history():
    ...
    try:
        rf = (sys.modules.get("Vera.vera.research_fabric")
...
if _VERA_MODE:
    try:
        ...
        from Vera.vera import research_fabric as _rf
    ...
    @capability(
        ...
        description="Submit a web research job that searches and synthesises information from the internet. "
                    "WHEN TO USE: when you need to find, investigate, or compile information about a topic from the web; "
                    "for 'research X', 'find out about Y', 'what is Z', 'trending topics', 'latest news on'. "
                    "This is a LONG-RUNNING tool — it returns {job_id, status} immediately; "
                    "the agent loop polls research.job.status until done, then reads research.job.result. "
                    "HOW TO USE: research.run(query='your question or topic', mode='single') — single is fast, "
                    "parallel is thorough (multiple agents), deep is most comprehensive. "
                    "Params: query (str!), mode (single|parallel|deep, default single), "
                    "output_mode (report|guide|code|filestore, default report), project_id (str). "
                    "Output: {job_id, status} — use research.job.status(job_id=...) to check completion.",
        schema={"properties": {
    ...
    @capability("research.report", http_method="POST", http_path="/research/report",
                ...
                description="Fast web research that produces a markdown report. "
                            ...
                            "'what is the latest on' — faster than research.parallel/deep (single agent). "
                            "LONG-RUNNING: returns {job_id, status} immediately; "
                            "poll research.job.status until status='done', then call research.job.result. "
                            "Input: query (str!), project_id (str), context (str — background info), context_mode (str). "
                            "Equivalent to research.run(mode='single', output_mode='report').",
    ...
    @capability("research.parallel", http_method="POST", http_path="/research/parallel",
                ...
                description="Multi-agent parallel web research with report output. "
                            ...
                            "multiple agents explore different angles simultaneously. Slower but more thorough than research.report. "
    ...
    @capability("research.deep", http_method="POST", http_path="/research/deep",
                ...
                description="Deep exhaustive research with report output — most thorough pipeline. "
                            ...
                            "long-form research where quality beats speed. "
                            "LONG-RUNNING: returns {job_id, status}; poll research.job.status until done, then read research.job.result. "
                            "Input: query (str!), project_id, context, context_mode.",
                schema=_ALIAS_SCHEMA)
    ...
    @capability("research.code", http_method="POST", http_path="/research/code",
                ...
                description="Research-then-implement pipeline: researches a topic then produces working code. "
                            ...
                            "'create a script that does Z'. Output is structured code with file paths. "
                            "LONG-RUNNING: returns {job_id, status}; poll research.job.status until done, then read research.job.result. "
                            "Input: query (str!), project_id, context, context_mode.",
                schema=_ALIAS_SCHEMA)
    ...
    @capability("research.guide", http_method="POST", http_path="/research/guide",
                ...
                description="Web research formatted as a structured step-by-step guide. "
                            ...
                            "'step-by-step instructions for'. Output is numbered sections with clear steps. "
                            "LONG-RUNNING: returns {job_id, status}; poll research.job.status until done, then read research.job.result. "
                            "Input: query (str!), project_id, context, context_mode.",
                schema=_ALIAS_SCHEMA)
    ...
    @capability("research.filestore", http_method="POST", http_path="/research/filestore",
                ...
                description="Deep research that produces a complete project file tree with full content. "
                            ...
                            "'generate a project scaffold with all files'. Output includes file paths + content for every file. "
                            "LONG-RUNNING: returns {job_id, status}; poll research.job.status until done, then read research.job.result. "
                            "Input: query (str!), project_id, context, context_mode.",
                schema=_ALIAS_SCHEMA)
    ...
    @capability(
        ...
        http_tags=["research"], memory="off",
        description="Read the completed output of a research job. "
                    "CALL AFTER research.job.status returns status='done'. "
                    "Input: job_id (str). "
                    "Output: {job_id, status, result (full markdown text or code), output_mode, "
                    "citations: [{url, title, snippet}], query, elapsed}. "
                    "For code/filestore jobs, result contains structured file paths + content.",
        schema={"properties": {"job_id": {"type": "string"}}},
    )
