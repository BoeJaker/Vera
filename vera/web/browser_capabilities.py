...
async def _new_page(browser):
    ...
    ctx  = await browser.new_context(
        ...
        # Vera itself serves HTTPS on a self-signed cert (§2.24's preview
        # route in particular — a step verifying its OWN sandbox file hits
        # THIS Vera instance's own :8999). Without this, Playwright's real
        # Chromium correctly refuses the untrusted cert
        # (net::ERR_CERT_AUTHORITY_INVALID), which is exactly right for
        # browsing the open internet but wrong for a browser session whose
        # whole point is to check Vera's own local content back to itself.
...
async def browser_navigate(
    ...
    # Observed live: a specialist reaching for a more structured call shape
    # (e.g. passing `url`/`actions` instead of `goal`) hit a hard
    # "missing 1 required positional argument: 'goal'" TypeError — a crash
    # before this cap's own logic ever ran, indistinguishable to the caller
    # from any other failure, and costly (repeated identical-call blocking
    # then burns a whole extra cycle before the specialist tries something
    # else). `goal` is genuinely required to plan sensible actions, but a
    # blank one plus a real `start_url` is still enough to attempt something
    # useful rather than crash outright.
    if not goal.strip():
        goal = (f"Extract the key information relevant to this page from "
                 f"{start_url}." if start_url else
                 "Extract the key information from the current page.")
    ...
    SYSTEM = (
        ...
        "IMPORTANT: for click/type, COPY the exact `selector=\"...\"` value shown for "
        "that element in 'Page elements' below — do NOT invent, shorten, or rewrite "
        "your own CSS selector from the element's description. The shown selector is "
        "guaranteed to work (it was generated by inspecting the real DOM); a selector "
        "you compose yourself from the visible label often isn't valid CSS and will "
        "fail. If the element you need isn't listed, scroll to reveal it first. "
        ...
        try:
            ...
            for step_i in range(max_steps):
                ...
                page_summary: str = await page.evaluate("""() => {
                    // Build a selector that's actually guaranteed to work, rather than
                    // making the model guess CSS from a text description — observed
                    // live: an element with no id/class fell back to the bare literal
                    // "button" (matches EVERY button on the page), so the model tried
                    // to invent its own attribute/text-based selector instead and
                    // produced invalid CSS (e.g. an unescaped space inside an id
                    // selector) or a selector for an attribute that isn't there.
                    // Priority: #id > [aria-label=...] > [title=...] > Playwright's
                    // own :has-text() text-match extension (works via page.query_selector
                    // even though it isn't standard CSS) > the bare tag as a last resort.
                    function esc(s) { return CSS.escape(String(s||'')); }
                    function bestSelector(el, tag) {
                        if (el.id) return '#' + esc(el.id);
                        const aria = (el.getAttribute('aria-label') || '').trim();
                        if (aria) return tag + "[aria-label='" + aria.replace(/'/g, '').slice(0,60) + "']";
                        const title = (el.getAttribute('title') || '').trim();
                        if (title) return tag + "[title='" + title.replace(/'/g, '').slice(0,60) + "']";
                        const text = (el.innerText || el.value || el.textContent || '').trim().slice(0, 40);
                        // Single-quoted (Playwright accepts either) — the display line
                        // below wraps the whole selector in DOUBLE quotes
                        // (selector="..."), so a double-quoted value here would
                        // collide with that outer delimiter and truncate early.
                    ...
                    document.querySelectorAll('input,textarea,select').forEach((el,i) => {
                        ...
                        const sel = bestSelector(el, el.tagName.toLowerCase());
                        if (n) parts.push(`INPUT[${t}] name="${n}" selector="${sel}"`);
                    });
                    // Buttons
                    document.querySelectorAll('button,[role=button],input[type=submit]').forEach(el => {
                        const t = (el.innerText||el.value||el.textContent||'').trim().slice(0,60);
                        if (t) parts.push(`BUTTON "${t}" selector="${bestSelector(el, 'button')}"`);
                    });
                    // Top links
