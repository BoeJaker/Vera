...
(function(){
  ...
  var COL = {
    ...
    _edge_default:       'rgba(150,170,190,0.9)',
    // Memory edge colors
  ...
  // Real vs inferred edge classification — drives solid (real connections:
  // discovery + entity extraction + structure + memory) vs dashed (inferred:
  // loom stitching, 3rd-order, similarity). Used by the renderer AND by the
  // loom list-row swatches so the two always agree.
  var _INFERRED_REL = /^(SIMILAR_TO|RELATED_TO|SHARES_TOPIC|DEPENDS_ON|DERIVED_FROM)$/i;
  var _REAL_REL = /^(HAS_ENTITY|MENTIONED_IN|CONTAINS|LINKS_TO|CO_OCCURS|REFERENCES)$/i;
  function isInferredEdge(e){
    if (!e) return false;
    if (e._dashed || e.dashed) return true;            // explicit inferred flag
    var layer = e._layer || (e.props && (e.props.layer || e.props.group)) || '';
    if (layer === 'loom' || layer === '3rd_order') return true;
    if (layer === 'entities' || layer === 'discover' ||
        layer === 'structure' || layer === 'memories') return false;
    var rel = e.rel || '';
    if (_INFERRED_REL.test(rel)) return true;
    if (_REAL_REL.test(rel)) return false;
    return false;   // unknown → treat as a real connection (solid, visible)
  }
  ...
  function createGraph(container, opts){
        ...
        (opts.showLayerToggle && opts.layers && opts.layers.length > 1 ?
          '<select class="vg-layer" style="font-size:10px;padding:2px 6px;background:var(--bg2);color:var(--text);border:1px solid var(--border);border-radius:3px">' +
            opts.layers.map(function(l){
              var label = ({fabric:'Fabric',entity:'Entities',aux:'Aux',memory:'Memory',net:'Network',
                entities:'Entities',discover:'Discover',structure:'Structure',
                memories:'Memories',loom:'Loom','3rd_order':'3rd Order'})[l] || l;
    ...
    // Theme colours come from CSS custom properties via getComputedStyle, which
    // forces a synchronous style recalc on every call. Resolving them inside the
    // per-node / per-edge draw loops meant thousands of forced reflows per frame
    // on large graphs. Cache by var name and only invalidate when the theme
    // actually changes (the vera:theme message below clears it).
    var _themeCache = {};
    function _themeColor(v, fallback) {
      var hit = _themeCache[v];
      if (hit !== undefined) return hit;
      var val = getComputedStyle(canvas).getPropertyValue(v).trim();
      if (!val) val = getComputedStyle(document.documentElement).getPropertyValue(v).trim();
      return (_themeCache[v] = (val || fallback));
    }
    ...
    if (typeof ResizeObserver !== 'undefined') {
      new ResizeObserver(function(){ resize(); state._dirty = true; if (!state.stopped) { state.frozen = false; state.tickCount = 0; } else { startTick(); } }).observe(canvas);
    }

    // The id→node map and degree map only change when the graph topology
    // changes, yet draw() and tick() run every frame at 60fps. Rebuilding them
    // each frame was O(n+E) of pure garbage. Cache them and rebuild only when
    // the node/edge counts change — every mutation here (push / filter / reset)
    ...
    function draw(){
      ...
      var nm = _tc.nm;
      var _deg = _tc.deg;
      var _STRUCTURAL_T = { Dataset:1, Source:1, Category:1, Ontology:1, Skill:1, Agent:1 };
      var _s = state.scale;

      // Theme colours used inside the draw loops, resolved once per frame.
      var _C_DIM2 = _themeColor('--dim2', '#8a7e70');
      var _C_TEXT = _themeColor('--text', '#ddd5c8');
      var _C_TEXT_DIM = _C_TEXT + 'bb';
      var _C_ACC  = _themeColor('--acc',  '#5a9e8f');
      var _C_ACC3 = _themeColor('--acc3', '#c9955a');
      // Label culling: rendering fillText per node/edge dominates on big graphs.
      // For large graphs only label nodes that are highlighted or large enough on
      ...
      state.edges.forEach(function(e){
        ...
        // SOLID  = real connections: discovery, entity extraction, structure,
        ...
        if (isSel) {
          edgeAlpha = 1; dashPat = isInferred ? [4, 3] : null;
        } else if (isInferred) {
          dashPat = [5 / _s, 4 / _s];
          edgeAlpha = isHub ? 0.3 : 0.5;
        } else {
          dashPat = null;   // real → solid
          edgeAlpha = isStructural ? 0.85 : (isHub ? 0.45 : 0.75);
        }

        // Fade edges whose endpoints are below the relevance threshold, mirroring
        // the node fade so "irrelevant" parts of the graph recede together.
        if (!isSel && _minRel > 0) {
          var relA = _nodeRel(a), relB = _nodeRel(b);
          var edgeRel = Math.min(relA, relB);
          if (edgeRel < _minRel) {
            var _deficit = (_minRel - edgeRel) / _minRel;
            edgeAlpha *= Math.max(0.05, 1 - 0.88 * _deficit);
          }
        }
      ...
      state.nodes.forEach(function(n){
        ...
        // anyway and fillText is what's killing the frame rate.
        var _showLabel = state.scale > 0.4 && (!_bigGraph || hov || sel || hl || (r * _s) >= 5);
        if (_showLabel) {
          ctx.font = '9px monospace';
          ctx.fillStyle = (hov || sel) ? _C_TEXT : _C_TEXT_DIM;
          ctx.textAlign = 'center';
        ...
        var _EXPANDABLE = { Dataset:1, FabricRecord:1, Entity:1, Ontology:1, Skill:1 };
        var _isExpanded = state.expanded[n.id] && state.expanded[n.id].length;
        // Datasets with dot-children get a distinct filled indicator even before expansion
        var _hasDotChildren = n.type === 'Dataset' && n.props && n.props.has_children;
        if (_EXPANDABLE[n.type]) {
          var _ix = n.x + r * 0.72, _iy = n.y - r * 0.72;
          ctx.beginPath();
          ctx.arc(_ix, _iy, 3.5, 0, Math.PI * 2);
          if (_isExpanded) {
            ctx.fillStyle = _C_ACC;
            ctx.fill();
          } else if (_hasDotChildren) {
            ctx.fillStyle = _C_ACC3;
            ctx.fill();
          } else {
    ...
    function tick(){
      ...
      if (state.frozen) {
        // A settled graph is static, so repainting it ~10×/s forever is pure
        // waste — and with several graph instances live on the page at once that
        // cost multiplies. Only repaint when something has marked the view dirty
      ...
      if (state._axisMode) {
        // Spatial-hash grid for the bounded declutter — the shove only acts
        // within ~50px (r2 > 2600 is skipped), so a CELL=60 grid + 3×3
        // neighbourhood captures every interacting pair while staying ~O(n)
        // at thousands of nodes (latent-map layouts can be very large).
        var _AX_CELL = 60;
        var _axGrid = {};
        for (var _axi = 0; _axi < state.nodes.length; _axi++) {
          var _axn = state.nodes[_axi];
          var _axk = Math.floor(_axn.x / _AX_CELL) + ',' + Math.floor(_axn.y / _AX_CELL);
          (_axGrid[_axk] || (_axGrid[_axk] = [])).push(_axn);
        }
        ...
        for (var ai = 0; ai < state.nodes.length; ai++) {
          ...
          var _axcx = Math.floor(a.x / _AX_CELL), _axcy = Math.floor(a.y / _AX_CELL);
          for (var _aox = -1; _aox <= 1; _aox++) {
          for (var _aoy = -1; _aoy <= 1; _aoy++) {
            var _axcell = _axGrid[(_axcx + _aox) + ',' + (_axcy + _aoy)];
            if (!_axcell) continue;
            for (var _aci = 0; _aci < _axcell.length; _aci++) {
            var b = _axcell[_aci];
            if (a === b) continue;
      ...
      // ── Repulsion (spatial-hash grid: ~O(n) instead of O(n²)) ──────────────
      // The repulsion force min(60, 1200/d²) is already negligible (<0.09)
      // beyond ~120px, so a node only needs to repel against others in its own
      // grid cell and the 8 neighbouring cells. With CELL=120 every pair closer
      // than 120px on both axes is guaranteed to be considered; more distant
      // pairs contribute force below the noise floor. This is what keeps the
      // layout responsive at thousands of nodes (4k nodes: ~16M pair-checks/
      // frame collapses to roughly linear).
      var _REPEL_CELL = 150;
      var _grid = {};
      for (var _gi = 0; _gi < state.nodes.length; _gi++) {
        var _gn = state.nodes[_gi];
        var _gk = Math.floor(_gn.x / _REPEL_CELL) + ',' + Math.floor(_gn.y / _REPEL_CELL);
        (_grid[_gk] || (_grid[_gk] = [])).push(_gn);
      }
      state.nodes.forEach(function(a){
        ...
        var acx = Math.floor(a.x / _REPEL_CELL), acy = Math.floor(a.y / _REPEL_CELL);
        for (var ox = -1; ox <= 1; ox++) {
          for (var oy = -1; oy <= 1; oy++) {
            var cell = _grid[(acx + ox) + ',' + (acy + oy)];
            if (!cell) continue;
            for (var ci3 = 0; ci3 < cell.length; ci3++) {
              var b = cell[ci3];
              if (a === b) continue;
              var dx = a.x - b.x, dy = a.y - b.y;
              var d2 = dx * dx + dy * dy + 1;
              var d = Math.sqrt(d2);
              // Degree-weighted repulsion: a high-degree hub shoves its nearby
              // (often low-degree) neighbours outward so a dense neighbourhood
              // inflates instead of collapsing into a blob. Keyed on the *other*
              // node's degree, so for sparse graphs (deg ≤ 3) this is identical
              // to the previous min(60, 1200/d²).
              var _kb = (b._deg || 1) - 3;
              var _str = _kb > 0 ? 1800 + 320 * _kb : 1800;
              var _cap = _kb > 0 ? 90 + 32 * _kb : 90;
              var f = Math.min(_cap, _str / d2);
              // Hard minimum-spacing (collision): when two nodes overlap or
              // nearly touch, add a firm separation impulse so a dense blob
              // can't collapse onto a single point. This is what de-clutters
              // the unreadable central cluster.
              var _minGap = (a.r || 8) + (b.r || 8) + 8;
              if (d < _minGap) f += (_minGap - d) * 0.6;
              a.vx += f * dx / d;
              a.vy += f * dy / d;
            }
          }
      ...
      state.edges.forEach(function(e){
        ...
        var es = opts.edgeStyleFn ? opts.edgeStyleFn(e) : null;
        var restLen = (es && es.springLength) || 95;
        ...
        // Degree-scaled rest length: a hub's neighbours can't all sit 80px away —
        // they pile up into a blob. Give edges touching a high-degree node a
        // longer rest length so the hub fans its neighbours out onto a wider ring
        // (radius grows ~√degree, matching the circumference needed to hold them).
        // sqrt keeps it sub-linear so even huge hubs stay bounded. Sparse graphs
        // (degree ≤ 4) are unaffected.
        var hubDeg = Math.max(a._deg || 1, b._deg || 1);
        if (hubDeg > 4) restLen += Math.min(280, (Math.sqrt(hubDeg) - 2) * 36);
    ...
    function _revive(){
      try { resize(); } catch(e){}
      // resize() can reset the canvas backing store, so force at least one
      // repaint even if the graph is frozen (the frozen loop now only draws
      // when dirty) — otherwise a revived settled graph could come back blank.
      state._dirty = true;
    ...
    if (typeof IntersectionObserver !== 'undefined') {
      try {
        new IntersectionObserver(function(entries){
          var e = entries[entries.length - 1];   // latest visibility state
          if (e.isIntersecting) {
            _revive();
          } else {
            // Scrolled out of the viewport or display:none'd (e.g. a panel/tab
            // the user isn't looking at). There's nothing to see and no way to
            // interact, so halt the loop completely — this is what lets many
            // graph instances coexist without each one burning a frame budget.
            stopTick();
          }
    ...
    try {
      setInterval(function(){
        ...
        if (vis && !_wasVisible) {
          ...
          _revive();
        } else if (!vis && _wasVisible) {
          // Transition: visible → hidden. Halt the loop so a hidden instance
          // costs nothing (mirrors the IntersectionObserver stop, for cases it
          // doesn't catch such as an offsetParent going display:none).
          stopTick();
    ...
    function _renderLayerUI(){
      ...
      // Preferred display order for known layers
      var _LAYER_ORDER = ['entities','loom','discover','structure','memories','3rd_order',
                          'default','structural','informational','memory','worldview'];
      function rowHTML(l){
        var rec = state.layers[l];
        var vis = rec.visible !== false, phys = rec.physics !== false;
        var label = _LAYER_LABELS[l] || l;
        return '<div style="display:flex;align-items:center;gap:5px;padding:2px 3px;font-size:9px;font-family:var(--mono,monospace)">' +
          ...
          '<span style="flex:1;color:var(--text,#ddd5c8)'+(vis?'':';opacity:.5')+';overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+esc(label)+'</span>' +
      ...
      if (listEl) {
        ...
        var _sortedLayers = meaningful.slice().sort(function(a,b){
          var ai = _LAYER_ORDER.indexOf(a), bi = _LAYER_ORDER.indexOf(b);
          if (ai < 0) ai = 999; if (bi < 0) bi = 999;
          return ai !== bi ? ai - bi : a < b ? -1 : a > b ? 1 : 0;
        });
        listEl.innerHTML = _sortedLayers.map(rowHTML).join('');
        wireToggles(listEl);
        return;
      }
    ...
    async function expandNode(node){
      ...
      try {
        ...
        if (node.type === 'Dataset') {
          // First check for dot-hierarchy children (e.g. project.sub when expanding project)
          var _dotChildren = [];
          try {
            var _dcr = await fetch(apiBase + '/fabric/datasets?parent=' + encodeURIComponent(node.id));
            var _dcd = await _dcr.json();
            _dotChildren = (_dcd && _dcd.datasets) || [];
          } catch(e){}
          if (_dotChildren.length) {
            var _dcNodes = _dotChildren.map(function(d){
              var shortLabel = d.dataset_id.split('.').pop() || d.dataset_id;
              return { id: d.dataset_id, name: shortLabel, type: 'Dataset',
                props: { id: d.dataset_id, record_count: d.record_count,
                         has_children: d.has_children, layer: 'structure' }};
            });
            _ingest(_dcNodes, [], 'CONTAINS');
          } else {
          // No dot-children — expand to records from /fabric/browse
          ...
          // Also surface the dataset this record belongs to: add it as a node
          // (or, if it's already on the graph, just connect to it). This lets the
          // Loom flow drill entity → records → dataset by double-clicking.
          var recDsId = (node.props && (node.props.dataset_id || node.props.parent_dataset)) || '';
          if (recDsId) {
            if (!existing[recDsId]) {
              var dsNode = {
                id: recDsId, label: String(recDsId.split('.').pop() || recDsId).slice(0, 50),
                type: 'Dataset', props: { id: recDsId, layer: 'structure' },
                x: node.x + (Math.random() - 0.5) * 140,
                y: node.y + (Math.random() - 0.5) * 140,
                vx: 0, vy: 0, r: 14,
              };
              state.nodes.push(dsNode); state.nodeIndex[recDsId] = dsNode;
              existing[recDsId] = true; added.push(recDsId);
            }
            var hasDsEdge = state.edges.some(function(e){
              return e.rel === 'CONTAINS' && ((e.from === recDsId && e.to === node.id) || (e.from === node.id && e.to === recDsId));
            });
            if (!hasDsEdge) state.edges.push({ from: recDsId, to: node.id, rel: 'CONTAINS', props: {} });
          }
    ...
    var _DEFAULT_NODE_LAYER_MAP = {
      // structure: dataset/record hierarchy, infrastructure, schema containers
      Dataset:'structure', Surface:'structure', Subtable:'structure',
      Work:'structure', Container:'structure', Ontology:'structure',
      DAG:'structure', Skill:'structure', Category:'structure',
      DockerHost:'structure', NetHost:'structure', SshHost:'structure',
      Subnet:'structure', NetService:'structure',
      // discover: raw crawl output — pages, sources, records
      FabricRecord:'discover', Page:'discover', Source:'discover',
      // entities (default): named entities, agents, concepts
      Entity:'entities', Agent:'entities', CapResult:'entities',
      WorldviewPoint:'entities', Concept:'entities',
      // memories: memory trail, sessions
      Memory:'memories', Session:'memories',
      // 3rd_order: synthesis/distilled concept models
      TopicModel:'3rd_order', Concept:'3rd_order',
      // loom is set explicitly via node.layer property
    };
    // Edge layers are set explicitly by the system that created them (loom, discover,
    // entity extraction, 3rd_order, etc.) via edge.layer or edge.props.layer.
    // No type-based fallback — edges without an explicit layer land in 'default'.
    var _DEFAULT_EDGE_LAYER_MAP = {};
    // Human-readable labels for layer names (old names kept for backward compat)
    var _LAYER_LABELS = {
      entities:'Entities', discover:'Discover', structure:'Structure',
      memories:'Memories', loom:'Loom', '3rd_order':'3rd Order',
      default:'Default',
      // backward compat aliases
      structural:'Structure', informational:'Entities',
      memory:'Memories', worldview:'Concepts', fabric:'Fabric',
    };
    ...
    function _computeClusters(){
      ...
      nodes.forEach(function(n){
        ...
        // Degree drives the de-blob forces (longer springs + stronger repulsion
        // around hubs). Stored here so the per-frame force loop reads it directly.
        n._deg = nbrs.length;
      });
    ...
    function load(data){
      ...
      (data.nodes || []).forEach(function(n){
        ...
        if (!added) return;
        if (n.x !== undefined || n.y !== undefined) _explicitPos[added.id] = 1;
        if (prevPos[added.id] && n.x === undefined && n.y === undefined) {
      ...
      _computeClusters();
      // ── Initial spread (phyllotaxis seed) ────────────────────────────────
      // A fresh, highly-connected graph whose nodes all spawn near the centre
      // settles into an uninterpretable blob — the simulation can't climb out of
      // that minimum just by nudging. Seed any freshly-laid-out graph onto an
      // even sunflower disc instead, with the highest-degree nodes nearest the
      // centre. Springs then pull connected nodes together from a spread start,
    ...
    async function fetchSnapshot(layer, params){
      ...
      if (layer === 'entity') {
        ...
        if (params.dataset_id)   qs.push('dataset_id=' + encodeURIComponent(params.dataset_id));
        // When no explicit label_filter and no dataset scope, default to structural root
        // nodes only so the graph starts collapsed — children expand on demand.
        var lf = params.label_filter;
        if (!lf && !params.dataset_id) {
          lf = ({ fabric: 'Dataset,Source,Category,Ontology,Skill,Agent,DAG',
                  net:    'NetHost,SshHost,Subnet,NetService,Container,DockerHost' })[layer] || '';
        }
        if (lf) qs.push('label_filter=' + encodeURIComponent(lf));
      }
      try {
        var res = await fetch(apiBase + url + '?' + qs.join('&'));
        var data = await res.json();
        if (data && data.nodes) {
          // For structure/fabric views, fetch has_children flags in parallel
          var _dsChildMap = {};
          if (!params.dataset_id) {
            try {
              var _dcRes = await fetch(apiBase + '/fabric/datasets');
              var _dcData = await _dcRes.json();
              (_dcData && _dcData.datasets || []).forEach(function(d){
                _dsChildMap[d.dataset_id] = d.has_children;
              });
            } catch(e){}
          }
          var nodes = data.nodes.map(function(n){
            ...
            var mc = (n.props && n.props.mention_count) || 1;
            var props = n.props || {};
            if (nodeType === 'Dataset' && _dsChildMap[n.id] !== undefined) {
              props = Object.assign({}, props, { has_children: _dsChildMap[n.id] });
            }
            return {
              id:    n.id,
              label: (n.name || (props.title || props.name || props.url) || n.label || n.id || '').slice(0, 50),
              type:  nodeType,
              props: props,
    ...
    (function(){
      ...
      function _setActive(id, force){
        // `force` is used by programmatic openers (showTable/showContent) so that
        // re-selecting the already-active panel REFRESHES it instead of toggling
        // the drawer shut \u2014 otherwise clicking a second node (or a node while the
        // Table/Content tab is already open) would collapse the drawer and look
        // like "nothing renders".
        if (!force && _activeBd === id && !_collapsed) {
          // toggle off
          _collapsed = true;
          bdPanelsEl.classList.add('collapsed');
          _collapseEl.textContent = '\u25b2';
      ...
      _registerPanel({
        ...
        build: function(body, inst){
          ...
          // overrides that toggle, leaving every panel stacked & visible at once
          // (so only the first, Terminal, showed and Table/Content were clipped).
          body.style.flexDirection = 'column'; body.style.height = '100%';
          body.innerHTML =
      ...
      // ── Built-in panel: Chat (talk to Vera, runs relevant caps) ──────────
      // Wired to the ReAct agent loop (dag.agent_loop): the LLM auto-selects
      // relevant capabilities for each message, executes them, and returns a
      ...
      _registerPanel({
        ...
        build: function(body, inst){
          ...
          body.style.flexDirection = 'column'; body.style.height = '100%';
          body.innerHTML =
            '<div style="flex:1;overflow-y:auto;padding:8px 10px;font-size:10px;line-height:1.55" class="vg-bd-chat-log"></div>' +
            '<div style="display:flex;gap:5px;padding:5px 8px;border-top:1px solid var(--border,#3a3530);background:var(--bg1,#1f1d1a);flex-shrink:0">' +
              '<input class="vg-bd-chat-in" placeholder="Ask Vera about the data, or tell it what to do…" ' +
                'style="flex:1;min-width:0;font-size:10px;padding:4px 7px;background:var(--bg0,#181614);border:1px solid var(--border,#3a3530);color:var(--text,#ddd5c8);border-radius:3px;font-family:var(--mono,monospace)">' +
              '<button class="vg-bd-chat-send" style="font-size:9px;padding:4px 10px;background:rgba(90,158,143,.12);border:1px solid var(--acc,#5a9e8f);color:var(--acc,#5a9e8f);border-radius:3px;cursor:pointer">Send</button>' +
            '</div>';
          _chatBodyEl = body;
          var inp = body.querySelector('.vg-bd-chat-in');
          var btn = body.querySelector('.vg-bd-chat-send');
          if (_chatMsgs.length) _chatRenderAll();
          else _chatRenderAll(true);
          btn.onclick = _chatSend;
          inp.addEventListener('keydown', function(e){ if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); _chatSend(); } });
        }
      });

      function _chatRenderAll(hintOnly){
        if (!_chatBodyEl) return;
        var log = _chatBodyEl.querySelector('.vg-bd-chat-log'); if (!log) return;
        if (hintOnly && !_chatMsgs.length) {
          log.innerHTML = '<div style="color:var(--dim,#6a6058);font-size:9px">Ask about the loaded data (e.g. "summarise this dataset", "which entities matter most?") or give an instruction ("crawl example.com", "stitch related records"). Vera picks and runs the relevant capabilities.</div>';
          return;
        }
        log.innerHTML = _chatMsgs.map(function(m){
          if (m.role === 'user') {
            return '<div style="margin:6px 0 2px"><span style="color:var(--acc2,#8fb87a);font-weight:600">you</span> <span style="color:var(--text,#ddd5c8)">' + esc(m.text) + '</span></div>';
          }
          if (m.role === 'pending') {
            return '<div style="margin:2px 0;color:var(--dim,#6a6058)"><span style="color:var(--acc,#5a9e8f);font-weight:600">vera</span> ' + esc(m.text) + '</div>';
          }
          var steps = '';
          if (m.steps && m.steps.length) {
            steps = '<details style="margin-top:3px"><summary style="cursor:pointer;color:var(--dim,#6a6058);font-size:8.5px">' + m.steps.length + ' step' + (m.steps.length>1?'s':'') + '</summary>' +
              '<div style="margin-top:2px;font-size:8.5px;color:var(--dim2,#8a7e70);font-family:var(--mono,monospace)">' +
              m.steps.map(function(s){ return '→ ' + esc(s); }).join('<br>') + '</div></details>';
          }
          var txt = esc(m.text || '');
          txt = txt.replace(/(https?:\/\/[^\s<"]+)/g, function(u){ return '<a href="' + esc(u) + '" target="_blank" rel="noopener" style="color:var(--acc,#5a9e8f)">' + esc(u) + '</a>'; });
          return '<div style="margin:2px 0 6px"><span style="color:var(--acc,#5a9e8f);font-weight:600">vera</span> <span style="color:var(--text,#ddd5c8);white-space:pre-wrap">' + txt + '</span>' + steps + '</div>';
        }).join('');
      ...
      async function _chatSend(){
        if (!_chatBodyEl) return;
        var inp = _chatBodyEl.querySelector('.vg-bd-chat-in');
        var btn = _chatBodyEl.querySelector('.vg-bd-chat-send');
        var msg = (inp.value || '').trim(); if (!msg) return;
        inp.value = '';
        _chatMsgs.push({ role: 'user', text: msg });
        var pending = { role: 'pending', text: '…thinking (picking & running capabilities)' };
        _chatMsgs.push(pending);
        _chatRenderAll();
        if (btn) btn.disabled = true;
        try {
          var res = await fetch(apiBase + '/dag/agent_loop', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ goal: msg, allowed_caps: '', max_cycles: 6, session_id: _chatSession }),
          });
          var data = await res.json();
          // Drop the pending placeholder
          var pi = _chatMsgs.indexOf(pending); if (pi >= 0) _chatMsgs.splice(pi, 1);
          if (data && !data.error) {
            var summary = data.summary || (data.final_state && (data.final_state.answer || data.final_state.summary)) || 'Done.';
            var steps = [];
            (data.history || []).forEach(function(h){
              var cap = h.capability || h.action || (h.action_input && h.action_input.capability) || '';
              if (cap && cap !== 'done') steps.push(cap);
            });
            _chatMsgs.push({ role: 'vera', text: String(summary), steps: steps });
          } else {
            _chatMsgs.push({ role: 'vera', text: 'Error: ' + ((data && data.error) || 'request failed') });
          }
        } catch (e) {
          var pj = _chatMsgs.indexOf(pending); if (pj >= 0) _chatMsgs.splice(pj, 1);
          _chatMsgs.push({ role: 'vera', text: 'Error: ' + (e && e.message || e) });
        } finally {
