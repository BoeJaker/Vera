      ...
      async function loadSubviews(skipAutoActivate){
        ...
        if(!skipAutoActivate && !st.activeView){
          ...
          if(summ.datasets.length || summ.nodeCount){
            // Try to find an existing match first — by shared dataset...
            ...
            // ...or, for graphs without dataset-tagged nodes (loom, fabric
            // structure, etc.), by significant overlap of node_ids with a
            // previously saved subview's node set.
            if(!match && summ.nodeIds.length){
              var bestOverlap=0;
              st.subviews.forEach(function(sv){
                var nids=(sv.meta&&sv.meta.node_ids)||[];
                if(!nids.length)return;
                var nidSet={};nids.forEach(function(id){nidSet[String(id)]=1;});
                var shared=0;
                summ.nodeIds.forEach(function(id){if(nidSet[String(id)])shared++;});
                var ratio=shared/Math.min(nids.length,summ.nodeIds.length);
                if(ratio>0.5 && ratio>bestOverlap){bestOverlap=ratio;match=sv;}
              });
            }
            if(match){
