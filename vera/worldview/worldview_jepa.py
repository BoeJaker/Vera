...
class WorldView:
    ...
    def _checkpoint_dict(self) -> Dict:
        ...
        # Persist the cached latent vectors + their record ids so the 2D
        # visualisation (worldview.snapshot) is ready on server start instead of
        # returning "train first" until the map is rebuilt from the fabric.
        # Capped so very large corpora don't bloat the checkpoint blob.
        try:
            cl = getattr(self, "_cached_latents", None)
            cr = getattr(self, "_cached_rids", None)
            if HAS_NUMPY and cl is not None and cr is not None and len(cr):
                _cap = 20000
                if len(cr) > _cap:
                    ckpt["cached_latents"] = np.asarray(cl[:_cap], dtype="float32")
                    ckpt["cached_rids"]    = list(cr[:_cap])
                else:
                    ckpt["cached_latents"] = np.asarray(cl, dtype="float32")
                    ckpt["cached_rids"]    = list(cr)
        except Exception as e:
            log.debug("worldview: could not cache latents into checkpoint: %s", e)
        return ckpt
    ...
    def _restore_from_ckpt(self, ckpt: Dict) -> bool:
        ...
        # Restore cached latents/rids so worldview.snapshot can render the latent
        # map immediately on startup without re-encoding the whole fabric.
        cl = ckpt.get("cached_latents")
        cr = ckpt.get("cached_rids")
        if HAS_NUMPY and cl is not None and cr:
            try:
                self._cached_latents = np.asarray(cl, dtype="float32")
                self._cached_rids    = list(cr)
            except Exception as e:
                log.debug("worldview: cached latent restore failed: %s", e)
        return True
...
def _get_fabric():
    import sys
    return (sys.modules.get("Vera.vera.fabric.data_fabric") or
            sys.modules.get("Vera.vera.fabric.data_fabric"))
...
def _restore_index_from_cached_latents():
    """Repopulate the in-memory FAISS index from latents restored with the
    checkpoint, so nearest-neighbour search (worldview.query) works on startup.
    The 2D snapshot already falls back to MODEL._cached_latents directly, so the
    latent map renders even if FAISS is unavailable."""
    try:
        cl = getattr(MODEL, "_cached_latents", None)
        cr = getattr(MODEL, "_cached_rids", None)
        if cl is None or not cr:
            return
        if WV_INDEX.available and WV_INDEX._index is not None and WV_INDEX._index.ntotal > 0:
            return  # already populated (e.g. by a fresh train)
        if WV_INDEX.available or WV_INDEX.connect():
            metas = [MODEL.record_meta.get(rid, {}) for rid in cr]
            WV_INDEX.add_batch(cr, cl, metas)
            log.info("worldview: FAISS index restored from cached latents (%d vectors)",
                     len(cr))
    except Exception as e:
        log.debug("worldview: index restore from cached latents failed: %s", e)
...
async def cap_worldview_reembed_missing(
    ...
    from Vera.vera.capability_orchestration import ollama_embed as _ollama_embed
