    ...
    async def _handle_event(self, event: dict):
        ...
        if etype == "cap.error":
            ...
            rec = SyslogRecord(
                ...
                detail       = event.get("traceback") or error,
                cap_name     = cap_name,
...
async def syslog_ask(
    ...
    entries_json: str  = "",   # JSON array of entry dicts (multi-select from the UI)
    agent_name:   str  = "assistant",
    trace_id=None,
):
    # 1. Multiple entries supplied directly from the UI (ticked in the list).
    entries: list = []
    if entries_json:
        try:
    ...
    # 2. Single entry by redis id.
    if not entries and log_id:
        r = _redis()
        if r:
            try:
    ...
    # 3. Fall back to the latest error (optionally for a given cap).
    if not entries:
        entries = await _read_syslog(
            limit=1, level="ERROR",
    ...
    # Build a context block per entry; pull source code once per distinct cap.
    blocks: list = []
    src_cache: dict = {}
    for i, entry in enumerate(entries):
        cn = entry.get("cap_name", cap_name)
        if cn and cn not in src_cache:
            src_cache[cn] = _resolve_cap_source(cn)
        src = src_cache.get(cn, {})
        label = f"Log entry {i + 1}/{len(entries)}" if len(entries) > 1 else "System log entry"
        block = f"""{label}:
  Timestamp:  {entry.get('ts','')}
  Level:      {entry.get('level','')}
  Capability: {entry.get('cap_name','')} ({entry.get('cap_group','')})
  Message:    {entry.get('message','')}
  Detail:     {(entry.get('detail','') or '')[:600]}
  ...
  File:       {entry.get('source_file', src.get('file',''))}"""
        ...
        # Only inline source for the first entry referencing each cap to keep the
        # prompt bounded when many entries are selected.
        if code and (len(entries) == 1 or i < 3):
            block += f"\n\nSource code:\n```python\n{code[:1200]}\n```"
        blocks.append(block)
    ...
    system = (
        ...
        "Given one or more log entries and optionally the source code of the failing "
        "capabilities, provide a clear diagnosis and specific remediation steps. "
        "When multiple entries are supplied, note any common root cause across them. "
    ...
    return {
        ...
        "count":      len(entries),
        "answer":     answer,
        "cap_name":   entries[0].get("cap_name", cap_name),
    }
