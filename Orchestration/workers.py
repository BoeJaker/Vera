...
async def _load_jobs():
    ...
    if r:
        try:
            ...
            done_ids = set(j.get("id","") for j in COMPLETED_JOBS[-200:] if j.get("id"))

            # Use xpending to get truly unacknowledged entries
            try:
                pinfo = await r.xpending(_orch.TASK_STREAM, _orch.GROUP_WORKERS)
                if pinfo and pinfo.get("pending", 0) > 0:
                    # Fetch detailed pending entries
                ...
                entries = await r.xrange(_orch.TASK_STREAM, count=200)
    ...
    return {"pending": pending, "running": running,
            "done": list(reversed(COMPLETED_JOBS[-100:]))}

@APP.get("/cluster/jobs")
async def route_jobs(): return await _load_jobs()
